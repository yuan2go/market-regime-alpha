...
from tests.contracts.decision_support.test_context_domain import _policy
...
@pytest.fixture
def context_stack(target_database_url, tmp_path, request):
...
def test_context_preparation_rejects_cross_run_identity(context_stack) -> None:
    stack = context_stack
    commands = ContextCommands(
        PostgresContextInputPreparationProvider(stack.pool),
        PostgresContextUnitOfWorkProvider(stack.pool),
