...
from tests.contracts.decision_support.test_qualification_roster_domain import (
    _uuid,
)
...
@pytest.fixture
def decision_qualification_stack(target_database_url, tmp_path, request):
    return _qualification.qualification_stack.__wrapped__(
        target_database_url,
        tmp_path,
        request,
    )
...
def test_exact_cutoff_resolver_rejects_wrong_purpose_and_superseded_revision(
    decision_qualification_stack,
) -> None:
    stack = decision_qualification_stack
