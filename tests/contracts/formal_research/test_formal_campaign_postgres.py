...
from tests.contracts.decision_support import test_decision_postgres as _decision
from tests.contracts.decision_support import (
    test_decision_closure_postgres as _closure,
)
from tests.contracts.decision_support import (
    test_strategy_inference_postgres as _inference,
)
from tests.contracts.decision_support.test_decision_closure_domain import (
    _portfolio_policy,
    _risk_policy,
)
from tests.contracts.research_qualification import test_research_postgres as _research
from tests.contracts.selection import (
...
@pytest.fixture
def formal_campaign_stack(target_database_url, tmp_path, request):
...
def test_campaign_predeclaration_is_atomic_exact_and_replayable(
    formal_campaign_stack,
) -> None:
    definition = _campaign_definition(formal_campaign_stack)
    commands = FormalCampaignCommands(
        PostgresFormalCampaignUnitOfWorkProvider(formal_campaign_stack.pool),
        id_factory=uuid4,
    )
    runtime, _ = _candidate._schedule_run(
        formal_campaign_stack,
    ...
    with psycopg.connect(formal_campaign_stack.database_url) as connection:
...
def test_campaign_unknown_commit_recovers_exact_authority(
    ...
    recovered = FormalCampaignCommands(
        _UnknownCommitCampaignProvider(formal_campaign_stack.pool),
        id_factory=uuid4,
    ).predeclare(
        definition,
...
def test_campaign_mid_root_failure_leaves_no_partial_roster_and_recovers(
    ...
    assert PostgresFormalCampaignQueryPort(formal_campaign_stack.pool).verify(
        definition.formal_research_campaign_id
    ).matched is True
...
def test_rehearsal_decision_can_bind_but_cannot_create_formal_visibility(
    ...
    provider_commands = ProviderQualificationCommands(
        PostgresProviderQualificationUnitOfWorkProvider(
            formal_campaign_stack.pool, id_factory=uuid4
        ),
        id_factory=uuid4,
    )
    ...
    with pytest.raises(RuntimeStateConflictError):
        provider_commands.admit_trading_session_visibility(
            ...
            session_id=formal_campaign_stack.market_session_id,
...
def test_campaign_materializes_exact_rosters_and_opens_protected_before_access(
    ...
    with psycopg.connect(formal_campaign_stack.database_url) as connection:
        connection.execute(
            ...
            WHERE session_id = %s
            """,
            (uuid4(), formal_campaign_stack.market_session_id),
        )
        connection.commit()
    campaign_commands = FormalCampaignCommands(
        PostgresFormalCampaignUnitOfWorkProvider(formal_campaign_stack.pool),
    ...
    for runtime_profile, builder in (
        ...
        _, runtime_run_id = _candidate._schedule_run(
            formal_campaign_stack,
            steps=steps,
            canonical_decision_time=formal_campaign_stack.decision_time,
        )
    ...
    experiment_commands = ExperimentCommands(
        PostgresExperimentUnitOfWorkProvider(formal_campaign_stack.pool),
        id_factory=uuid4,
    )
    ...
    with formal_campaign_stack.pool.connection(read_only=True) as connection:
...
def test_rehearsal_campaign_cannot_resolve_formal_pit_or_register_formal_dataset(
    ...
    with pytest.raises(RuntimeStateConflictError):
        PostgresFormalPitSourceReadPort(formal_campaign_stack.pool).resolve_exact(
            ...
            source_identity=formal_campaign_stack.market_fact_revision_id,
            requested_decision_time=formal_campaign_stack.decision_time.value,
        )

    feature_suffix = uuid4().hex[:8]
    feature = replace(
        _research._dataset_feature(formal_campaign_stack),
        feature_code=f"wp14_formal_feature_{feature_suffix}",
    )
    formal_campaign_stack.research.register_feature_definition(
        feature,
    ...
    with pytest.raises(RuntimeStateConflictError):
        formal_campaign_stack.research.register_formal_dataset(
            dataset,
            scope,
            _context("register-formal-dataset"),
        )
    ordinary = formal_campaign_stack.research.register_dataset(
        dataset,
