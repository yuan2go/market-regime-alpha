...
def test_unobserved_outcome_after_grace_refuses_settlement_and_preserves_other_work(monkeypatch):
    from market_regime_alpha.interfaces import daily_service
    from market_regime_alpha.interfaces.daily_research import encode_daily_plan
    ...
    work = DailyOutcomeWorkItem(
        ...
        schedule_code="daily-outcome-" + frozen.experimental_model_use_id.hex,
        fire_key="daily-outcome:" + str(frozen.prediction_id),
        parent_run_id=frozen.runtime_run_id, run_state="QUEUED", requested_at=frozen.decision_time,
        code_sha=frozen.code_sha, config_sha256=sha256(content).hexdigest(), plan_content=content, error_code=None,
    )
    reads = SimpleNamespace(
        validate_configuration=lambda _: None, outcome_work_items=lambda **_: (work,),
        ready=lambda _: SimpleNamespace(target_window_end=frozen.decision_time-timedelta(hours=9)),
        now=lambda: frozen.decision_time,
        target_price_members=lambda _: (DailyInputMember(UUID(int=1), DailyInputState.MISSING, "NO_EXACT_SESSION_BAR"),),
        missing_elapsed_session_pairs=lambda _: ((UUID(int=21), UUID(int=22)),),
        observe=lambda _: SimpleNamespace(content_sha256="d"*64),
    )
    def forbidden(*args, **kwargs):
        raise AssertionError("unobserved Outcome must not claim settlement or silently retry Provider")
    monkeypatch.setattr(daily_service, "_collection", forbidden)
    monkeypatch.setattr(daily_service.DailyResearchOperations, "settle_and_evaluate", forbidden)
    monkeypatch.setattr(daily_service, "_abstain", lambda *args: {"state": "ABSTAINED"})
    result = daily_service.daily_tick(SimpleNamespace(daily_prediction_reads=reads), frozen, None,
        worker_id="daily", maximum_steps=1, before_action=lambda: None)
    assert result["state"] == "ABSTAINED"
    assert result["pending"][0]["state"] == "OUTCOME_DATA_UNOBSERVED"
    assert result["pending"][0]["reason_code"] == "OUTCOME_COLLECTION_GRACE_EXPIRED"
    assert result["pending"][0]["run_id"] == work.run_id
    assert result["pending"][0]["automatic_retry"] is False
