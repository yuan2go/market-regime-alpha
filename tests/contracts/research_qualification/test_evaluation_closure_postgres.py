...
from tests.contracts.outcome import test_outcome_postgres as _outcome


@pytest.fixture
def evaluation_stack(target_database_url, tmp_path, request):
...
def test_protocol_formula_companion_round_trips_and_reconciles(evaluation_stack) -> None:
...
def test_evaluation_run_can_fail_once_and_replays_exact_failure(evaluation_stack) -> None:
    stack = evaluation_stack
    target, _, _, settled = _settle_two_visible_revisions(stack)
    commands, _, experiment_run_id, protocol = _freeze_and_predeclare(stack, target)
    evaluation_run_id = uuid4()
...
def test_evaluation_run_rejects_a_future_knowledge_cutoff(evaluation_stack) -> None:
    stack = evaluation_stack
    target, _, _, _ = _settle_two_visible_revisions(stack)
    commands, _, experiment_run_id, protocol = _freeze_and_predeclare(stack, target)
...
def test_database_rejects_a_revision_not_visible_at_run_cutoff(evaluation_stack) -> None:
    stack = evaluation_stack
...
def test_not_due_acquisition_fails_without_access_or_observation(evaluation_stack) -> None:
    stack = evaluation_stack
    target = _outcome._register_midnight_target(stack)
...
def test_unavailable_outcome_member_is_retained_as_not_estimable(evaluation_stack) -> None:
    stack = evaluation_stack
    target = _outcome._register_midnight_target(stack)
    commitment_id = _outcome._open_decision(stack, target)
