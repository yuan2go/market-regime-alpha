...
def _input_paths(tmp_path: Path, fixture: DailyDecisionFixture):
    ...
    daily_manifest_path = publish_controlled_source_manifest(
        root=tmp_path / "daily-manifests", artifact=daily_manifest
    )
    all_sessions = tuple(
        sorted(
            set(
    ...
    configuration = ControlledOperationRuntimeConfiguration.create(
        static_feature_set=static_technical_feature_set(
            effective_from=decision - timedelta(days=365)
        ),
        ...
        research=ControlledResearchPipelineConfig.create(
            candidate_discovery=ControlledCandidateDiscoveryConfig.create(
                top_n=5, minimum_candidate_population=5
            )
        ),
...
def _minute_payload(symbol: str, market_date: date) -> bytes:
    ...
    rows = [
        f"{1440 + index:04d} {10 + index / 1000:.3f} {100 + index * 100} {100000 + index * 100000}"
...
def test_controlled_runner_uses_real_canonical_chain_and_is_idempotent(
    ...
    inputs, calendar, universe, configuration, decision = _input_paths(
    ...
    runner = ControlledDecisionTimeOperationRunner(
        journal=SQLiteDecisionTimeOperationJournal(
            tmp_path / "controlled-operation.sqlite3", clock=lambda: observed
        ),
