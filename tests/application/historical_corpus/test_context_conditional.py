...
def _external_evidence() -> HistoricalResearchEvidence:
    return HistoricalResearchEvidence.create(
        ...
        metrics=(),
        payload={"qualification_status": "SUPPORTED"},
...
def test_session_level_context_cannot_be_packaged_as_within_session_interaction() -> None:
    ...
    observations = (
        ContextObservation(date(2026, 1, 2), "A", Decimal("1"), Decimal(".01"), "BULL", Decimal("1"), _ref("RESEARCH_PANEL", "panel-a")),
        ContextObservation(date(2026, 1, 2), "B", Decimal("2"), Decimal(".02"), "BEAR", Decimal("0"), _ref("RESEARCH_PANEL", "panel-b")),
    )

    with pytest.raises(ValueError, match="constant within each session"):
        evaluate_context_conditioning(
            definition, observations=observations, top_k=1, expected_population=2
        )
...
def test_session_context_evaluates_performance_across_sessions_only() -> None:
    ...
    observations = tuple(
        ContextObservation(
            ...
            _ref("RESEARCH_PANEL", f"panel-{day}-{index}"),
        )
        for day, label in ((2, "BULL"), (3, "BEAR"), (4, "BULL"), (5, "BEAR"))
        for index in range(4)
    )

    result = evaluate_context_conditioning(
        definition, observations=observations, top_k=2, expected_population=16
    )
...
def test_cross_sectional_context_supports_interaction_and_incremental_information() -> None:
    ...
    result = evaluate_context_conditioning(
        definition, observations=observations, top_k=2, expected_population=20
    )
...
def test_cross_sectional_context_without_within_session_variation_is_not_estimable() -> None:
    ...
    observations = tuple(
        ContextObservation(date(2026, 1, day), f"S{index}", Decimal(index), Decimal(index), "SAME", Decimal("1"), _ref("RESEARCH_PANEL", f"panel-{day}-{index}"))
...
def test_session_context_with_identical_alpha_effect_is_neutral() -> None:
    ...
    result = evaluate_context_conditioning(
        ...
        top_k=2,
        expected_population=len(observations),
    )
