...
def test_finish_stage_fault_injection_rolls_back_every_journal_mutation(
    ...
    path = tmp_path / f"{fault_point}.postgres-scope"
    armed = False
    ...
    repository = PostgresLifecycleRunRepository(path, fault_injector=inject)
    run, stage, attempt = _claimed_started(repository, _command(tmp_path))
    transition = _transition(run, stage, attempt)
    armed = True
    ...
    recovered = PostgresLifecycleRunRepository(path).finish_stage(transition)
...
def test_database_guards_run_identity_and_terminal_history(tmp_path: Path) -> None:
    ...
    def rejected(statement: str, parameters: tuple[object, ...], match: str) -> None:
        with postgres_connection(repository.path) as connection:
            with pytest.raises(psycopg.Error, match=match):
                connection.execute(statement, parameters)

    rejected(
        "UPDATE lifecycle_runs SET command_hash = %s WHERE run_id = %s",
        (HASH_B, str(run.run_id)),
        "command identity",
    )
    rejected(
        "DELETE FROM lifecycle_runs WHERE run_id = %s",
        (str(run.run_id),),
        "cannot be deleted",
    )
    rejected(
        "UPDATE lifecycle_attempts SET result = result WHERE attempt_id = %s",
    ...
    rejected(
        "DELETE FROM lifecycle_stages WHERE run_id = %s AND stage_name = %s",
        (str(run.run_id), LifecycleStageName.SIGNAL.value),
        "cannot be deleted",
    )
    rejected(
        "DELETE FROM lifecycle_stage_receipts WHERE receipt_id = %s",
    ...
    with postgres_connection(repository.path) as connection:
        ...
        with pytest.raises(psycopg.IntegrityError, match="foreign key"):
            connection.execute(
                ...
                ) VALUES (%s, %s, %s, 'ATTEMPT_STARTED', %s, %s, NULL, '{}', '{}', %s, %s, %s)
                """,
                (
                    "cross-run-event",
                    str(run1.run_id),
