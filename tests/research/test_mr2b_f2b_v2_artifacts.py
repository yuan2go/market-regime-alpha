...
from market_regime_alpha.research.mr2b_f2b_v2 import F2BResultsV2
from market_regime_alpha.research.mr2b_f2b_v2_artifacts import (
    build_f2b_v2_run_identity,
    publish_f2b_v2_artifact,
)
...
def _insufficient_results() -> F2BResultsV2:
    ...
    competing = CompetingEventResultV2(
...
def _publish(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    final = publish_f2b_v2_artifact(output_root=tmp_path / "runs", identity=identity, results=results)
    monkeypatch.setattr("market_regime_alpha.research.mr2b_f2b_v2_reader.frozen_f2b_v2_protocol", lambda: results.protocol)
    monkeypatch.setattr("market_regime_alpha.research.mr2b_f2b_v2_reader.build_f2b_v2_results", lambda **_: results)
...
def test_insufficient_run_publishes_empty_fixed_schema_distributions(
    ...
    verified = load_verified_f2b_v2_run(final, dataset=dataset, mr1=mr1, f2a=f2a)
...
def test_checksum_valid_v2_semantic_tamper_fails(
