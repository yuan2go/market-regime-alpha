-- Immutable exploratory model training journal and executable parameter store.
-- These rows cannot qualify PIT, OOS, Calibration or Production.  Formal
-- Forecast computation may only reload an AVAILABLE artifact after the
-- independent Formal PIT gate has succeeded.
...
CREATE TABLE research_model_training_request (
    ...
    experiment_definition_id text,
    experiment_definition_hash text CHECK (
        experiment_definition_hash IS NULL
        OR experiment_definition_hash ~ '^sha256:[0-9a-f]{64}$'
    ),
    ...
    payload_json jsonb NOT NULL CHECK (
        ...
        AND (
            payload_json->>'schema_version' = 'research-model-training-request/v1'
            OR (
                experiment_definition_id IS NOT NULL
                AND experiment_definition_hash IS NOT NULL
                AND payload_json->'experiment_definition'->>'definition_id' = experiment_definition_id
                AND payload_json->'experiment_definition'->>'definition_hash' = experiment_definition_hash
                AND jsonb_typeof(payload_json->'measure_bindings') = 'array'
                AND jsonb_array_length(payload_json->'measure_bindings') > 0
            )
        )
    ),
    requested_at timestamptz NOT NULL,
    CHECK ((experiment_definition_id IS NULL) = (experiment_definition_hash IS NULL))
);
...
CREATE INDEX research_model_training_sample_lookup_idx
ON research_model_training_sample(request_id, trading_date, symbol);
...
CREATE TABLE research_model_artifact (
    ...
    model_parameter_hash text CHECK (
        model_parameter_hash IS NULL
        OR model_parameter_hash ~ '^sha256:[0-9a-f]{64}$'
    ),
    ...
    payload_json jsonb NOT NULL CHECK (
        jsonb_typeof(payload_json) = 'object'
        AND payload_json->>'schema_version' = 'research-model-artifact/v2'
        AND payload_json->>'model_parameter_hash' IS NOT DISTINCT FROM model_parameter_hash
    ),
    trained_at timestamptz NOT NULL,
    CHECK (research_model_available = (status = 'AVAILABLE')),
    CHECK ((status = 'AVAILABLE') = (model_parameter_hash IS NOT NULL))
);

CREATE INDEX research_model_artifact_parameter_idx
ON research_model_artifact(model_parameter_hash)
WHERE model_parameter_hash IS NOT NULL;
...
CREATE INDEX research_model_inference_lookup_idx
ON research_model_inference_receipt(artifact_id, symbol, decision_time);
