"""PostgreSQL-native repository for atomic complete-account Portfolio/Risk assessments."""
...
class PostgresCompleteAccountPortfolioRiskRepository(NativePostgresRepository):
    """Native PostgreSQL complete-account Portfolio/Risk authority."""
    ...
    def save_assessment(
        ...
        *,
        ...
        with self._connect() as connection:
            acquire_scope_lock(
                connection,
                namespace="complete-account-risk",
                identity=account_snapshot.account_id,
            )
            try:
...
def _insert_risk(
    connection: PostgresConnection, risk: CompleteAccountRiskDecision
) -> None:
    existing = connection.execute(
        """
        SELECT decision_json FROM complete_account_risk_decisions
        WHERE risk_decision_id = %s OR portfolio_decision_id = %s
        """,
...
def _load_snapshot(
    connection: PostgresConnection, snapshot_id: ArtifactId
) -> AuthoritativeAccountPortfolioSnapshot:
...
def _load_risk(
    connection: PostgresConnection, risk_id: RiskDecisionId
) -> CompleteAccountRiskDecision:
    row = connection.execute(
        """
...
__all__ = ["PostgresCompleteAccountPortfolioRiskRepository"]
