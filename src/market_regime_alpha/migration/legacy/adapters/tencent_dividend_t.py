"""Inject accepted Tencent composite frames into the Legacy dividend-T path."""
    ...
    def minute_bars(
        ...
        *,
        ...
        if symbol not in self._frames:
            raise KeyError(
                f"symbol not accepted by composite quality gate: {symbol}"
            )
...
def refresh_dividend_t_from_composite(
    *,
    ...
    """Refresh unchanged dividend-T logic from accepted composite frames."""
    ...
    if any("candidate_rank" in row for row in snapshot.get("rows", ())):
        raise RuntimeError(
            "Candidate rank must remain separate from dividend-T actions"
        )
...
def compare_dividend_snapshots(
    ...
    """Compare display fields without reinterpreting strategy semantics."""
