"""Decision-model contract capped at human-in-the-loop decision semantics."""
...
class DecisionModelResult:
    ...
    def __post_init__(self) -> None:
        ...
        _require_input_references(self.input_artifact_ids, self.input_hashes)
        _require_aware_time(self.as_of_time)
        if not isinstance(self.state, DecisionOutcome):
            raise TypeError("state must be a DecisionOutcome")
