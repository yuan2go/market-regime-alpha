"""PostgreSQL-native adapter for decision aggregates and append-only histories."""
...
class PostgresDecisionLifecycleRepository(NativePostgresRepository):
    """Native PostgreSQL implementation of DecisionLifecycleRepository."""
    ...
    def create_opportunity(
        ...
        *,
        ...
        with self._connect() as connection:
            acquire_scope_lock(
                connection,
                namespace="decision-opportunity",
            ...
            try:
                ...
                existing = connection.execute(
                    "SELECT aggregate_json FROM trading_opportunities WHERE opportunity_id = %s",
        ...
        with self._connect() as connection:
            ...
            try:
                ...
                if connection.execute(
                    "SELECT 1 FROM trading_theses WHERE opportunity_id = %s",
                    (str(opportunity.opportunity_id),),
        ...
        with self._connect() as connection:
            ...
            try:
                ...
                updated = connection.execute(
                    ...
                    SET state = %s, aggregate_json = %s, version = %s
    ...
    def _result_from_command(
        self, connection: PostgresConnection, row: dict[str, Any]
    ) -> DecisionCommandResult:
        opportunity = (
            _load_opportunity(
                connection,
...
def _load_opportunity(
    ...
    row = connection.execute(
        "SELECT * FROM trading_opportunities WHERE opportunity_id = %s",
        (str(opportunity_id),),
    ).fetchone()
    if row is None:
    ...
    event_rows = connection.execute(
        ...
        WHERE opportunity_id = %s AND sequence <= %s ORDER BY sequence
        """,
...
def _load_thesis(
    ...
    row = connection.execute(
        "SELECT * FROM trading_theses WHERE thesis_id = %s", (str(thesis_id),)
    ).fetchone()
    if row is None:
    ...
    event_rows = connection.execute(
        ...
        WHERE thesis_id = %s AND sequence <= %s ORDER BY sequence
        """,
...
def _command(connection: PostgresConnection, key: str) -> dict[str, Any] | None:
    return connection.execute(
        "SELECT * FROM decision_commands WHERE idempotency_key = %s", (key,)
    ).fetchone()


def _validate_command(row: dict[str, Any], command_hash: str) -> None:
