"""Transactional native PostgreSQL repository for the canonical Lifecycle Runtime Journal."""
...
from market_regime_alpha.application.canonical_lifecycle._postgres_codec import (
...
class PostgresLifecycleRunRepository(NativePostgresRepository):
    ...
    def create_or_get(
        ...
        *,
        ...
        with self._transaction() as connection:
            acquire_scope_lock(
                connection,
                namespace="canonical-lifecycle-run",
            ...
            try:
                cursor = connection.execute(
                    ...
                    ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, NULL, %s, %s, %s, 1, 0, %s, %s, NULL)
            ...
            row = connection.execute(
                "SELECT * FROM lifecycle_runs WHERE idempotency_key = %s",
                (command.idempotency_key,),
            ).fetchone()
            if row is None:
                collision = connection.execute(
                    "SELECT * FROM lifecycle_runs WHERE run_id = %s",
                    (str(command.run_id),),
                ).fetchone()
    ...
    def get_stage(
        ...
        with self._connect() as connection:
            ...
            row = connection.execute(
                "SELECT * FROM lifecycle_stages WHERE run_id = %s AND stage_name = %s",
        ...
        *,
        ...
        with self._transaction() as connection:
            ...
            cursor = connection.execute(
                ...
                SET stage_status = %s, attempt_count = %s, stage_json = %s, version = %s
                WHERE run_id = %s AND stage_name = %s AND version = %s
    ...
    def finish_stage(self, transition: StageTransition) -> LifecycleRun:
        ...
        with self._transaction() as connection:
            ...
            existing_receipt_row = connection.execute(
                ...
                WHERE receipt_id = %s OR (
                    run_id = %s AND stage_name = %s AND receipt_hash = %s
                )
                """,
                (
    ...
    def history(self, run_id: LifecycleRunId) -> LifecycleHistory:
        ...
        with self._read_transaction() as connection:
            ...
            stage_rows = connection.execute(
                "SELECT * FROM lifecycle_stages WHERE run_id = %s",
                (str(run_id),),
            ).fetchall()
            attempt_rows = connection.execute(
                "SELECT * FROM lifecycle_attempts WHERE run_id = %s",
                (str(run_id),),
            ).fetchall()
            receipt_rows = connection.execute(
                """
                SELECT * FROM lifecycle_stage_receipts
                WHERE run_id = %s ORDER BY created_at, receipt_id
                """,
                (str(run_id),),
            ).fetchall()
            event_rows = connection.execute(
                ...
                WHERE run_id = %s ORDER BY sequence_number
                """,
                (str(run_id),),
            ).fetchall()
        stages = tuple(
    ...
    @contextmanager
    def _read_transaction(self) -> Iterator[PostgresConnection]:
        """Keep multi-query journal reads on one PostgreSQL transaction."""
    ...
    def _select_run(
        self, connection: PostgresConnection, run_id: LifecycleRunId
    ) -> dict[str, Any]:
        row = connection.execute(
            "SELECT * FROM lifecycle_runs WHERE run_id = %s", (str(run_id),)
        ).fetchone()
        if row is None:
    ...
    def _select_attempt(
        self, connection: PostgresConnection, attempt_id: LifecycleAttemptId
    ) -> dict[str, Any]:
        row = connection.execute(
            "SELECT * FROM lifecycle_attempts WHERE attempt_id = %s",
            (str(attempt_id),),
        ).fetchone()
        if row is None:
    ...
    def _complete_attempt(
        ...
        cursor = connection.execute(
            ...
            SET result = %s, attempt_json = %s, completed_at = %s,
                exception_type = %s, exception_message = %s
            WHERE attempt_id = %s AND result = 'RUNNING' AND claim_token = %s
            """,
            (
    ...
    def _update_run(
        ...
        *,
        ...
        cursor = connection.execute(
            ...
            SET status = %s, current_stage = %s, run_json = %s, version = %s,
                claim_token = %s, updated_at = %s, completed_at = %s
            WHERE run_id = %s AND version = %s AND claim_token = %s
            """,
            (
    ...
    def _append_event(
        ...
        *,
        ...
        if previous is not None:
            prior_created_at = _parse_timestamp(previous["created_at"])
        ...
        sequence_row = connection.execute(
            ...
            FROM lifecycle_events WHERE run_id = %s
            """,
            (str(run.run_id),),
        ).fetchone()
        if sequence_row is None:
            raise LifecycleJournalIntegrityError("event sequence query returned no row")
        sequence = int(sequence_row["sequence_number"])
        semantic = {
...
def _require_rowcount(cursor: psycopg.Cursor[Any], message: str) -> None:
