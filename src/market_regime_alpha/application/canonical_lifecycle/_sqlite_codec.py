...
def run_from_row(row: sqlite3.Row) -> LifecycleRun:
...
def command_from_row(row: sqlite3.Row) -> CanonicalLifecycleCommand:
    return _decode_record(
        str(row["command_json"]), CanonicalLifecycleCommand.from_canonical_dict
    )


def stage_from_row(row: sqlite3.Row) -> LifecycleStage:
...
def attempt_from_row(row: sqlite3.Row) -> LifecycleAttempt:
...
def receipt_from_row(row: sqlite3.Row) -> StageReceipt:
...
def event_from_row(row: sqlite3.Row) -> LifecycleEvent:
