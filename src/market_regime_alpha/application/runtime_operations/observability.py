    ...
    def to_canonical_dict(self) -> dict[str, Any]:
        return {
            ...
            "completed_at": (
                ...
                else canonical_datetime(self.completed_at)
            ),
            ...
            "lease_expires_at": (
                ...
                else canonical_datetime(self.lease_expires_at)
            ),
        }
    ...
    def trace_run(self, run_id: ArtifactId) -> dict[str, Any]:
        ...
        for tick in snapshot.ticks:
            ...
            observations.append(
                RuntimeStageObservation(
                    ...
                    duration_seconds=_duration(
                        tick.created_at, tick.completed_at
                    ),
            ...
            attempts = self._attempt_observations(
                run_id, tick.command.tick_id, tick.command.observed_at, trace_id
            )
            ...
            observations.append(
                RuntimeStageObservation(
                    ...
                    provider=None,
                    started_at=max(
                        item.stage_completed_at for item in summary.stages
                    ),
                    ...
                    retry_count=0,
                    coverage=(1.0 if not summary.missing_evidence else None),
                    deadline_margin_seconds=(
                        summary.decision_time
                        - max(
                            item.evidence_available_at for item in summary.stages
                        )
                    ).total_seconds(),
                    fencing_token=tick.fencing_token,
                    lease_expires_at=tick.lease_expires_at,
    ...
    def metrics(self, run_id: ArtifactId) -> dict[str, Any]:
        ...
        with self._factory.connection(read_only=True) as connection:
            ...
            candidate_count = connection.execute(
                """
                SELECT count(*) FROM state_runtime_candidate_artifact
                WHERE run_id = %s
                """,
                (str(run_id),),
            ).fetchone()
        if counts is None or candidate_count is None:
            raise RuntimeError("Runtime metrics aggregate returned no row")
        return {
            "schema_version": "canonical-runtime-metrics/v1",
            "run_id": str(run_id),
            ...
            "provider_failures": sum(
                item["status"] not in {"STARTED", "SUCCEEDED"}
                for item in providers
            ),
            "retries": sum(item["retry_count"] for item in providers),
            "deadline_misses": sum(
                (item["deadline_margin_seconds"] or 0) < 0
    ...
    def _attempt_observations(
        ...
        return tuple(
            RuntimeStageObservation(
                ...
                status=str(row[4]),
                reason_codes=tuple(
                    str(item) for item in json.loads(str(row[6]))
                ),
                retry_count=max(0, int(row[5]) - 1),
                coverage=1.0 if str(row[4]) == "SUCCEEDED" else None,
                deadline_margin_seconds=(
                    decision_time - (row[3] or row[2])
                ).total_seconds(),
...
def _event_count(events: tuple[Any, ...], tick_id: ArtifactId, event_type: str) -> int:
    return sum(
        ...
        and (item.tick_id is None or item.tick_id == tick_id)
        for item in events
    )
