"""PostgreSQL-native repository for Thesis-scoped manual execution traceability."""
...
class PostgresTraceableManualExecutionRepository(PostgresManualExecutionRepository):
    ...
    def create_traceable_trade(
        ...
        *,
        ...
        with self._connect() as connection:
            acquire_scope_lock(
                connection,
                namespace="position-book",
                identity=book.position_book_id,
            )
            try:
    ...
    def trades_for_book(
        ...
        with self._connect() as connection:
            rows = connection.execute(
                ...
                WHERE binding.position_book_id = %s
    ...
    def open_position_books(self, account_id: str) -> tuple[PositionBook, ...]:
        with self._connect() as connection:
            rows = connection.execute(
                ...
                "WHERE account_id = %s AND state = 'OPEN' "
        ...
        with self._connect() as connection:
            ...
            try:
                ...
                updated = connection.execute(
                    ...
                    SET state = %s, version = %s, aggregate_json = %s, closed_at = %s
                    WHERE position_book_id = %s AND version = %s AND state = 'OPEN'
                    """,
                    (
                        book.state.value,
                        book.version,
...
def _load_book(
    connection: PostgresConnection, book_id: PositionBookId
) -> PositionBook:
    row = connection.execute(
        "SELECT aggregate_json, version FROM position_books WHERE position_book_id = %s",
        (str(book_id),),
    ).fetchone()
    if row is None:
    ...
    events = connection.execute(
        ...
        "WHERE position_book_id = %s ORDER BY sequence",
        (str(book_id),),
    ).fetchall()
...
def _insert_book_event(
    connection: PostgresConnection, book: PositionBook, idempotency_key: str
) -> None:
...
def trace_binding_payload(
    ...
    row = connection.execute(
        "SELECT * FROM traceable_manual_trade_bindings WHERE manual_trade_id = %s",
        (str(trade_id),),
    ).fetchone()
    if row is None:
...
def _validate_stored_binding(
    connection: PostgresConnection, record: ManualTradeRecord
) -> None:
    row = connection.execute(
        "SELECT * FROM traceable_manual_trade_bindings WHERE manual_trade_id = %s",
        (str(record.manual_trade_id),),
...
def _validate_reducing_stored_binding(
    ...
    row = connection.execute(
        "SELECT * FROM risk_reducing_manual_trade_bindings WHERE manual_trade_id = %s",
        (str(record.manual_trade_id),),
    ).fetchone()
...
__all__ = ["PostgresTraceableManualExecutionRepository"]
