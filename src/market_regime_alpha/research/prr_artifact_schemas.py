    ...
    required_files=frozenset(
        {
            ...
            "pit_qualification.json",
            "partition_specification.json",
            "partition_seal.json",
            "partition_open_receipt.json",
            "data_quality.json",
            "amount_unit_contract.json",
            "cost_model.json",
            "universe_snapshots.parquet",
            "eligibility_snapshots.parquet",
            "orderability_snapshots.parquet",
            "candidate_populations.parquet",
            "feature_set.json",
            "candidate_feature_evidence.parquet",
            "model_spec.json",
            "candidate_model_scores.parquet",
            "candidate_rankings.parquet",
            "matched_k_selections.parquet",
            "matched_k_returns.parquet",
            "evaluation_marks.parquet",
...
PIT_REPLICATION_SUCCESS_RUN_SCHEMA = ArtifactSchema(
    schema_version="pit-candidate-replication-run-v1",
    required_files=frozenset(
        {
            "manifest.json",
    ...
    required_manifest_keys=PIT_REPLICATION_BLOCKED_RUN_SCHEMA.required_manifest_keys
    | frozenset({"row_counts"}),
)
