"""Protocol-driven directional Primary gate for F2B v3."""
...
def evaluate_primary_gate_v3(
    *,
    ...
    if required - evidence.keys():
        raise ValueError("F2B v3 Primary gate evidence is incomplete")
    ...
    if len(panels) != protocol.seed_panel_count:
        raise ValueError("F2B v3 seed-panel cardinality mismatch")
    checks = (
