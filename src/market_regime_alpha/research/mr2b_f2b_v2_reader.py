"""Semantic reader for immutable MR-2B F2B v2 evidence."""
...
from market_regime_alpha.research.mr2b_f2b_v2_artifacts import (
    F2B_V2_LIMITATIONS,
    F2BRunIdentityV2,
    PROJECT_ROOT,
)
...
from market_regime_alpha.research.prr_artifact_schemas import MR2B_F2B_V2_RUN_SCHEMA


@dataclass(frozen=True, slots=True)
class VerifiedF2BRunV2:
    root: Path
    run_id: str
    dataset_id: str
    mr1_run_id: str
...
def load_verified_f2b_v2_run(
    path: Path, *, dataset: VerifiedPRRDataset, mr1: VerifiedMR1Run, f2a: VerifiedF2ARun
) -> VerifiedF2BRunV2:
    root = path.resolve()
    _verify_files(root)
    manifest = _json(root / "manifest.json")
    if manifest.get("schema_version") != MR2B_F2B_V2_RUN_SCHEMA.schema_version:
        raise ValueError("F2B v2 schema mismatch")
    if frozenset(manifest.get("required_artifacts", ())) != MR2B_F2B_V2_RUN_SCHEMA.required_files:
        raise ValueError("F2B v2 required file contract mismatch")
    if MR2B_F2B_V2_RUN_SCHEMA.required_manifest_keys - manifest.keys():
        raise ValueError("F2B v2 manifest required fields are missing")
    identity = F2BRunIdentityV2.from_canonical_dict(_mapping(manifest.get("run_identity")))
    run_id = str(manifest.get("run_id"))
    if identity.run_id() != run_id or root.name != run_id:
        raise ValueError("F2B v2 Run ID mismatch")
    ...
    protocol = F2BProtocolV2(**protocol_payload)
    if protocol_id != protocol.protocol_id or protocol.to_canonical_dict() != frozen_f2b_v2_protocol().to_canonical_dict():
        raise ValueError("F2B v2 Protocol is not the frozen contract")
    if manifest.get("protocol_id") != protocol.protocol_id or identity.protocol_hash != protocol.protocol_id:
        raise ValueError("F2B v2 Protocol identity mismatch")
    expected = build_f2b_v2_results(dataset=dataset, mr1=mr1, f2a=f2a, protocol=protocol)
    ...
    for label in actual_json:
        if not _canonical_equal(actual_json[label], expected_json[label]):
            raise ValueError(f"F2B v2 {label} is not reconstructible")
    if tuple(_json_value(root / "limitations.json")) != F2B_V2_LIMITATIONS:
        raise ValueError("F2B v2 limitations mismatch")
    if manifest.get("row_counts") != _row_counts(actual_frames):
        raise ValueError("F2B v2 row counts mismatch")
    if manifest.get("statistics_executed") is not expected.statistics_executed:
        raise ValueError("F2B v2 coverage decision mismatch")
    if manifest.get("primary_assessment") != expected.primary_gate.assessment.value:
        raise ValueError("F2B v2 Primary assessment mismatch")
    if (
        manifest.get("dataset_id") != dataset.dataset_id
        or manifest.get("mr1_run_id") != mr1.run_id
        or manifest.get("f2a_run_id") != f2a.run_id
    ):
        raise ValueError("F2B v2 input identity mismatch")
    return VerifiedF2BRunV2(
...
def _verify_identity(
    identity: F2BRunIdentityV2, *, dataset: VerifiedPRRDataset, mr1: VerifiedMR1Run, f2a: VerifiedF2ARun
) -> None:
    if (
        ...
        raise ValueError("F2B v2 typed identity does not bind inputs")
    module_root = PROJECT_ROOT / "src" / "market_regime_alpha" / "research"
    expected = {name: _hash(module_root / name) for name in identity.implementation_module_hashes}
    if dict(identity.implementation_module_hashes) != expected:
        raise ValueError("F2B v2 implementation identity is stale")
...
def _expected_frames(results: F2BResultsV2) -> dict[str, pd.DataFrame]:
    empty = {
...
def _assert_table(actual: pd.DataFrame, expected: pd.DataFrame, keys: tuple[str, ...], label: str) -> None:
    if set(actual.columns) != set(expected.columns):
        raise ValueError(f"F2B v2 {label} columns mismatch")
    ...
    try:
        ...
        raise ValueError(f"F2B v2 {label} is not reconstructible") from exc
...
def _mapping(value: object) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise ValueError("F2B v2 run_identity must be a mapping")
    return value
