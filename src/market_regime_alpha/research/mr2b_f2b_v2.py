"""Application facade for coverage-first MR-2B F2B v2 evidence."""
...
from market_regime_alpha.research.mr2b_f2b_v2_statistics import (
...
F2B_V2_PRIMARY_OBSERVATION_SCHEMA_VERSION = "mr-2b-f2b-primary-observation-v2"
...
class F2BResultsV2:
    ...
    competing_events: CompetingEventResultV2
    ...
    def primary_observation_rows(self) -> tuple[dict[str, Any], ...]:
        return tuple(
            {
                "schema_version": F2B_V2_PRIMARY_OBSERVATION_SCHEMA_VERSION,
                **asdict(row),
    ...
    def primary_assessment_payload(self) -> dict[str, Any]:
        payload: dict[str, Any] = {
            "schema_version": "mr-2b-f2b-primary-assessment-v2",
...
def build_f2b_v2_results(
    *, dataset: VerifiedPRRDataset, mr1: VerifiedMR1Run, f2a: VerifiedF2ARun, protocol: F2BProtocolV2
) -> F2BResultsV2:
    if dataset.dataset_id != mr1.dataset_id or dataset.dataset_id != f2a.dataset_id:
        raise ValueError("F2B v2 Dataset identity chain mismatch")
    if mr1.run_id != f2a.mr1_run_id:
        raise ValueError("F2B v2 MR-1 identity chain mismatch")
    ...
    competing = build_competing_event_diagnostics_v2(
    ...
    if not coverage.sufficient_for_statistics:
        gate = evaluate_primary_gate_v2(coverage=coverage, evidence=None, protocol=protocol)
        return F2BResultsV2(
    ...
    gate = evaluate_primary_gate_v2(coverage=coverage, evidence=evidence, protocol=protocol)
...
def _secondary_inventory(
    ...
    *, dataset_id: str, mr1_run_id: str, f2a_run_id: str, protocol: F2BProtocolV2,
    ...
    if len(comparisons) != 108:
        raise ValueError("F2B v2 comparison family must contain exactly 108 comparisons")
    ...
    disclosure = {
        "schema_version": "mr-2b-f2b-multiple-testing-disclosure-v2",
...
def _empty_multiple_testing(protocol: F2BProtocolV2) -> dict[str, Any]:
    return {
        "schema_version": "mr-2b-f2b-multiple-testing-disclosure-v2",
        "comparison_count": 0,
        "primary_count": 0,
