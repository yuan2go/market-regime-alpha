"""Frozen single-source protocol for MR-2B F2B v3 hardening."""
...
F2B_V3_PROTOCOL_SCHEMA_VERSION = "mr-2b-f2b-protocol-v3"
...
class F2BProtocolV3:
    ...
    def __post_init__(self) -> None:
        if self.schema_version != F2B_V3_PROTOCOL_SCHEMA_VERSION:
            raise ValueError("unsupported F2B v3 Protocol schema")
        if self.alternative != "UP_GREATER_THAN_DOWN":
            raise ValueError("F2B v3 alternative is frozen")
        if self.eligible_context_labels != ("UP", "DOWN"):
            raise ValueError("F2B v3 labels are frozen")
        positive = (
        ...
        if any(value <= 0 for value in positive):
            raise ValueError("F2B v3 integer parameters must be positive")
        probabilities = (
        ...
        if any(not 0 < value <= 1 for value in probabilities):
            raise ValueError("F2B v3 probability parameters are invalid")
...
def frozen_f2b_v3_protocol() -> F2BProtocolV3:
    return F2BProtocolV3(
        schema_version=F2B_V3_PROTOCOL_SCHEMA_VERSION,
