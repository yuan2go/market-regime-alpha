...
def episode_prices(authority: MarketTargetOutcomeAuthority, entry_id: UUID, exit_id: UUID) -> OutcomeEpisodePrices:
    ...
    if (
        entry.status is not OutcomeStatus.COMPLETE
        or exit.status is not OutcomeStatus.COMPLETE
        or entry.close_value is None
        or exit.close_value is None
    ):
        raise ValueError("episode requires complete Outcome checkpoint prices")
    if (
        a.session_id != b.session_id
        or a.provider_product_id != b.provider_product_id
        or a.instrument_id != b.instrument_id
        or a.price_basis != b.price_basis
    ...
    return OutcomeEpisodePrices(
        ...
        entry_id,
        exit_id,
        bound[entry_id].market_target_outcome_observation_id,
        ...
        entry.close_value,
        exit.close_value,
        max(entry.known_at, exit.known_at),
