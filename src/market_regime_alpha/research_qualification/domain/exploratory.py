"""Explicit evidence lane and dual-clock scope for retrospective simulation."""
...
class ResearchEvidenceLane(StrEnum):
    EXPLORATORY_RETROSPECTIVE = "EXPLORATORY_RETROSPECTIVE"


@dataclass(frozen=True, slots=True)
class ExploratoryRetrospectiveDatasetScope:
    market_archive_id: UUID
    market_archive_seal_id: UUID
    knowledge_cutoff: datetime
    simulated_event_cutoff: datetime
    evidence_lane: ResearchEvidenceLane = field(
        default=ResearchEvidenceLane.EXPLORATORY_RETROSPECTIVE,
        init=False,
    )
