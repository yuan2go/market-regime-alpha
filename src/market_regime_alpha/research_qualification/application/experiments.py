...
from market_regime_alpha.research_qualification.ports.experiment_uow import ExperimentUnitOfWorkProvider
...
class ExperimentCommands:
    ...
    def _finish(self, uow: object, receipt_id: UUID, kind: str, aggregate_id: UUID, result_hash: str, context: CommandContext, runtime_claim: AttemptClaim | None) -> None:
        uow.receipts.succeed(receipt_id=receipt_id, aggregate_kind=kind, aggregate_id=str(aggregate_id), aggregate_version=1, result_hash=result_hash, runtime_claim=runtime_claim)  # type: ignore[attr-defined]
        uow.audit.append(audit_event_id=self._id_factory(), receipt_id=receipt_id, actor_type=context.actor_type.value, actor_id=context.actor_id, aggregate_kind=kind, aggregate_id=str(aggregate_id), action=f"{kind}_REGISTERED", reason_code=context.reason_code, before_version=None, after_version=1, runtime_claim=runtime_claim)  # type: ignore[attr-defined]
        if runtime_claim is not None:
            uow.runtime_finalization.succeed(runtime_claim, receipt_id=receipt_id, result_hash=result_hash)  # type: ignore[attr-defined]

    def _replay(self, uow: object, receipt: object, kind: str, runtime_claim: AttemptClaim | None) -> ExperimentMutationResult:
        ensure_replay_succeeded(receipt)  # type: ignore[arg-type]
        if receipt.result_aggregate_kind != kind or receipt.result_aggregate_id is None or receipt.result_hash is None:  # type: ignore[attr-defined]
            raise ArtifactIntegrityError("Experiment receipt is incomplete")
        aggregate_id = UUID(receipt.result_aggregate_id)  # type: ignore[attr-defined]
        record = uow.experiments.record(aggregate_id, lock=False) if kind == "EXPERIMENT" else uow.experiments.run_record(aggregate_id, lock=False)  # type: ignore[attr-defined]
        if canonical_json_sha256(record) != receipt.result_hash:  # type: ignore[attr-defined]
            raise ArtifactIntegrityError("Experiment receipt and Authority differ")
        if runtime_claim is not None:
            uow.runtime_finalization.succeed(runtime_claim, receipt_id=receipt.receipt_id, result_hash=receipt.result_hash)  # type: ignore[attr-defined]
            uow.commit()  # type: ignore[attr-defined]
        return ExperimentMutationResult(kind, aggregate_id, receipt.receipt_id, receipt.result_hash, True)  # type: ignore[attr-defined]
