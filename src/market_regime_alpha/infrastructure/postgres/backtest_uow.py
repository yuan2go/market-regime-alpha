"""One short PostgreSQL transaction for current Backtest predeclaration."""
...
class PostgresBacktestUnitOfWork:
    ...
    def __enter__(self) -> PostgresBacktestUnitOfWork:
        if self._used:
            raise RuntimeError("PostgresBacktestUnitOfWork cannot be nested or reused")
        self._used = True
    ...
    def backtests(self) -> PostgresBacktestRepository:
        return PostgresBacktestRepository(self._active())
...
class PostgresBacktestUnitOfWorkProvider:
    def __init__(self, pool: TargetPostgresPool) -> None:
        self._pool = pool

    def __call__(self) -> BacktestUnitOfWork:
        return PostgresBacktestUnitOfWork(self._pool)
