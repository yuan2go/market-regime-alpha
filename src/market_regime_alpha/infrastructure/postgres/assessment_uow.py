"""One short PostgreSQL transaction for ResearchAssessment Authority."""
...
from market_regime_alpha.infrastructure.postgres.repositories.research_assessments import (
...
class PostgresAssessmentUnitOfWork:
    ...
    def __enter__(self) -> PostgresAssessmentUnitOfWork:
        if self._used:
            raise RuntimeError("PostgresAssessmentUnitOfWork cannot be nested or reused")
        self._used = True
    ...
    def _active(self) -> psycopg.Connection[Any]:
        if self._connection is None:
            raise RuntimeError("PostgresAssessmentUnitOfWork is not active")
        return self._connection

    @property
    def assessments(self) -> PostgresAssessmentRepository:
    ...
    def __exit__(
        ...
        replacement = classify_research_postgres_error(exception, owner="Assessment")
...
class PostgresAssessmentUnitOfWorkProvider:
    def __init__(self, pool: TargetPostgresPool, *, id_factory: Callable[[], UUID]) -> None:
        self._pool = pool
        self._id_factory = id_factory

    def __call__(self) -> AssessmentUnitOfWork:
        return PostgresAssessmentUnitOfWork(
            self._pool, id_factory=self._id_factory
        )


__all__ = [
    "PostgresAssessmentUnitOfWork",
