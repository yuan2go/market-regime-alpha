"""One short PostgreSQL transaction for exploratory backtest predeclaration."""
...
from market_regime_alpha.infrastructure.postgres.repositories.exploratory_backtests import (
    PostgresExploratoryBacktestRepository,
)
...
class PostgresExploratoryBacktestUnitOfWork:
    ...
    def __enter__(self) -> PostgresExploratoryBacktestUnitOfWork:
        if self._used:
            raise RuntimeError(
                "PostgresExploratoryBacktestUnitOfWork cannot be nested or reused"
            )
        self._used = True
    ...
    def _active(self) -> psycopg.Connection[Any]:
        if self._connection is None:
            raise RuntimeError("Exploratory backtest UoW is not active")
        return self._connection

    @property
    def exploratory_backtests(self) -> PostgresExploratoryBacktestRepository:
        return PostgresExploratoryBacktestRepository(self._active())
...
class PostgresExploratoryBacktestUnitOfWorkProvider:
    def __init__(self, pool: TargetPostgresPool) -> None:
        self._pool = pool

    def __call__(self) -> ExploratoryBacktestUnitOfWork:
        return PostgresExploratoryBacktestUnitOfWork(self._pool)


__all__ = [
    "PostgresExploratoryBacktestUnitOfWork",
