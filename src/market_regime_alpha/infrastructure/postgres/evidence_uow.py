"""One short PostgreSQL transaction for Evidence Authority."""
...
from market_regime_alpha.infrastructure.postgres.repositories.research_evidence import (
...
class PostgresEvidenceUnitOfWork:
    ...
    def __enter__(self) -> PostgresEvidenceUnitOfWork:
        if self._used:
            raise RuntimeError("PostgresEvidenceUnitOfWork cannot be nested or reused")
        self._used = True
    ...
    def _active(self) -> psycopg.Connection[Any]:
        if self._connection is None:
            raise RuntimeError("PostgresEvidenceUnitOfWork is not active")
        return self._connection

    @property
    def evidence(self) -> PostgresEvidenceRepository:
        return PostgresEvidenceRepository(self._active())
    ...
    def __exit__(
        ...
        replacement = classify_research_postgres_error(exception, owner="Evidence")
...
class PostgresEvidenceUnitOfWorkProvider:
    def __init__(self, pool: TargetPostgresPool) -> None:
        self._pool = pool

    def __call__(self) -> EvidenceUnitOfWork:
        return PostgresEvidenceUnitOfWork(self._pool)


__all__ = ["PostgresEvidenceUnitOfWork", "PostgresEvidenceUnitOfWorkProvider"]
