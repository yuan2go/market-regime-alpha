...
def require_episode_parent(connection: psycopg.Connection[Any], evaluation_id: UUID,
                           sources: list[tuple[Any, ...]]) -> tuple[tuple[UUID, date], ...]:
    scope = connection.execute(
        """SELECT p.source_backtest_run_id, p.source_backtest_arm_id,
           ...
           WHERE e.evaluation_run_id = %s""", (evaluation_id,),
    ).fetchone()
    if (scope is None or any(value is None for value in scope[:4])
            or any(value is not None for value in scope[4:7]) or scope[7] != "VALIDATION"
            or not sources or {(row[13], row[14]) for row in sources} != {(scope[0], scope[1])}):
    ...
    slots = connection.execute(
        """SELECT session.exploratory_backtest_fold_session_id, session.session_date,
           ...
           ORDER BY session.session_date, session.exploratory_backtest_fold_session_id""", scope[:3],
    ).fetchall()
    ...
    if (not commitments or {row[1] for row in commitments} != {row[2] for row in slots}
           ...
           WHERE arm.exploratory_backtest_run_id = %s AND arm.exploratory_backtest_arm_id = %s
             ...
             AND cost.exploratory_backtest_arm_id IS NOT DISTINCT FROM
                 CASE WHEN arm.cost_binding_source = 'ARM_OVERRIDE'
                      THEN arm.exploratory_backtest_arm_id ELSE NULL::uuid END""", scope[:3],
    ).fetchall()
