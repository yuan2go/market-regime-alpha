...
class PostgresModelTrainingInputProvider:
    ...
    def prepare(
        ...
        for dataset_id, (definition, feature_definitions) in dataset_contracts.items():
            ...
            if verification.result != "VERIFIED":
                raise ArtifactIntegrityError(
                    f"training Dataset {dataset_id} manifest bytes are not exact"
                )
            ...
            if manifest.content_sha256 != binding.content_sha256:
                raise ArtifactIntegrityError(
                    f"training Dataset {dataset_id} manifest hash differs"
                )
        ...
        for ordinal, source in enumerate(source_rows, start=1):
            ...
            if manifest_row is None:
                raise ArtifactIntegrityError(
                    "FIT commitment instrument is absent from exact Dataset manifest"
                )
            cell_map = {
                item.feature_definition_id: item for item in manifest_row.cells
            }
            if not set(feature_ids).issubset(cell_map):
                raise ArtifactIntegrityError(
                    "FIT Dataset row omits a frozen Model feature"
                )
            ...
            for feature_id in feature_ids:
                ...
                if cell.status is not FeatureCellStatus.AVAILABLE:
                    ...
                    cell.value, (Decimal, int)
                ):
                    raise ArtifactIntegrityError(
                        "Model feature roster contains a non-numeric value"
                    )
                else:
            ...
            target_value = (
                None if source[14] is None else Decimal(str(source[14]))
            )
            ...
            artifact_rows.append(
                {
                    ...
                    "dataset_manifest": _artifact_payload(
                    ...
                    "feature_vector_sha256": (
                        None if vector_hash is None else str(vector_hash)
                    ),
                    ...
                    "state": state.value,
                    "target_value": (
                        None if sample.target_value is None else str(sample.target_value)
                    ),
                }
            )
        payload = {
            "evaluation_protocol_metric_id": str(
    ...
    def load_registered(
        ...
        if verification.result != "VERIFIED":
            raise ArtifactIntegrityError(
                "Model training input Artifact bytes are not exact"
            )
        content = self._byte_store.read_bytes(
            str(binding.content_sha256),
            expected_size=binding.size_bytes,
...
def _require_training_root(
    ...
    if row is None:
        raise RuntimeStateConflictError(
            "Model training requires exact completed FIT Evaluation/backtest parents"
        )
    features = tuple(
...
def _training_source_rows(
    ...
    if tuple(int(row[0]) for row in rows) != tuple(
        sorted(int(row[0]) for row in rows)
    ):
...
def _parse_registered_training_input(
    ...
    *,
    ...
    if not isinstance(payload, dict) or payload.get("schema") != "mra-model-training-input-v1":
    ...
    for raw in raw_samples:
        ...
        result.append(
            LinearTrainingRow(sample_id, tuple(values), Decimal(str(target_value)))
        )
