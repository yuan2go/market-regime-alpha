"""One short transaction for Model family/training/version mutations."""
...
from market_regime_alpha.infrastructure.postgres.repositories.research_models import (
...
class PostgresResearchModelUnitOfWork:
    ...
    def __enter__(self) -> PostgresResearchModelUnitOfWork:
        if self._used:
            raise RuntimeError("PostgresResearchModelUnitOfWork cannot be nested or reused")
        self._used = True
    ...
    def _active(self) -> psycopg.Connection[Any]:
        if self._connection is None:
            raise RuntimeError("Research Model UoW is not active")
        return self._connection

    @property
    def research_models(self) -> PostgresResearchModelRepository:
        return PostgresResearchModelRepository(self._active())
    ...
    def __exit__(
        ...
        replacement = classify_research_postgres_error(exception, owner="ResearchModel")
...
class PostgresResearchModelUnitOfWorkProvider:
    def __init__(self, pool: TargetPostgresPool) -> None:
        self._pool = pool

    def __call__(self) -> ResearchModelUnitOfWork:
        return PostgresResearchModelUnitOfWork(self._pool)


__all__ = [
    "PostgresResearchModelUnitOfWork",
