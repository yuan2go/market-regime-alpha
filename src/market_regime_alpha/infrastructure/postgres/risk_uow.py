"""One short fence-first PostgreSQL transaction for Risk Authority."""
...
from market_regime_alpha.decision_support.ports import RiskArtifactRepository, RiskUnitOfWork
from market_regime_alpha.infrastructure.postgres.pool import TargetPostgresPool
from market_regime_alpha.infrastructure.postgres.queries.decision_risk_inputs import PostgresRiskDependencyRepository
...
class PostgresRiskUnitOfWork:
    ...
    def __enter__(self) -> PostgresRiskUnitOfWork:
        if self._used:
            raise RuntimeError("PostgresRiskUnitOfWork cannot be nested or reused")
        self._used = True
    ...
    def _active(self) -> psycopg.Connection[Any]:
        if self._connection is None:
            raise RuntimeError("PostgresRiskUnitOfWork is not active")
        return self._connection

    @property
    def risks(self) -> PostgresRiskRepository:
        return PostgresRiskRepository(self._active())

    @property
    def dependencies(self) -> PostgresRiskDependencyRepository:
        return PostgresRiskDependencyRepository(self._active())

    @property
    def artifacts(self) -> RiskArtifactRepository:
        return cast(RiskArtifactRepository, PostgresTargetArtifactRepository(self._active()))
...
class PostgresRiskUnitOfWorkProvider:
    def __init__(self, pool: TargetPostgresPool) -> None:
        self._pool = pool

    def __call__(self) -> RiskUnitOfWork:
        return cast(RiskUnitOfWork, PostgresRiskUnitOfWork(self._pool))
...
def _replacement(exception: BaseException | None) -> BaseException | None:
    ...
    if exception.sqlstate.startswith(("22", "23")) or exception.sqlstate == "55000":
        return DecisionAuthorityIntegrityError("PostgreSQL rejected Risk invariants")
    return None
...
def _raise_commit_error(exc: psycopg.Error) -> None:
    ...
    if isinstance(exc, psycopg.OperationalError):
        raise DecisionCommitOutcomeUnknownError("PostgreSQL acknowledgement was lost during Risk commit") from exc
    raise exc


__all__ = ["PostgresRiskUnitOfWork", "PostgresRiskUnitOfWorkProvider"]
