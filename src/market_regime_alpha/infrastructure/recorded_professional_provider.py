...
class RecordedProfessionalMarketProvider:
    def __init__(self,contract: ProfessionalDailyContract, content: bytes, *, expected_sha256: str) -> None:
...
def replay_professional_capture(content: bytes) -> tuple[ProfessionalDailyContract, bytes, dict]:
    ...
    if not isinstance(raw,dict) or not isinstance(raw.get("schema"),str) or raw["schema"] not in {"mra-recorded-professional-capture-v1","mra-recorded-professional-capture-v2"}:
        raise ValueError("unsupported recorded Capture envelope")
    version_two=raw["schema"]=="mra-recorded-professional-capture-v2"
    if set(raw)!=(fields|{"request"} if version_two else fields):
