"""PostgreSQL-native repositories for Model Registry and Experiment Governance."""
...
class PostgresModelRegistryRepository(NativePostgresRepository):
        ...
        with self._connect() as connection:
            ...
            try:
                ...
                existing = connection.execute(
                    "SELECT * FROM model_registrations WHERE model_id = %s",
        ...
        with self._connect() as connection:
            ...
            try:
                ...
                updated = connection.execute(
                    ...
                    SET registration_json = %s, lifecycle_status = %s,
                        evidence_level = %s, version = %s
    ...
    def _load_at(
        ...
        transition_rows = connection.execute(
            ...
            WHERE model_id = %s AND sequence <= %s ORDER BY sequence
            """,
...
class PostgresExperimentGovernanceRepository(NativePostgresRepository):
    ...
    def create(
        ...
        *,
        ...
        with self._connect() as connection:
            acquire_scope_lock(
                connection,
                namespace="experiment-governance",
                identity=protocol.experiment_id,
            )
            try:
                command = _command(connection, idempotency_key)
                if command is not None:
                    ...
                    return result
                existing = connection.execute(
                    "SELECT * FROM governed_experiments WHERE experiment_id = %s",
        ...
        with self._connect() as connection:
            ...
            try:
                ...
                updated = connection.execute(
                    ...
                    SET validation_access_count = %s,
                        sealed_test_access_count = %s, version = %s
    ...
    def _load_at(
        ...
        events = connection.execute(
            ...
            WHERE experiment_id = %s AND sequence <= %s ORDER BY sequence
            """,
...
def _experiment_row(
    connection: PostgresConnection, experiment_id: ExperimentId
) -> dict[str, Any]:
    row = connection.execute(
        "SELECT * FROM governed_experiments WHERE experiment_id = %s",
        (str(experiment_id),),
...
def _command(
    ...
    return connection.execute(
        "SELECT * FROM governance_commands WHERE idempotency_key = %s",
        (idempotency_key,),
    ).fetchone()
