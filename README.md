...
- 当前已具备可恢复的探索性每日 Runtime Journal、Source Manifest 与质量门、B0/B1 PredictionRun、Phase D 每日决策 Artifact、CandidateRecommendation 投影、禁止 `ENTER` 的 Entry plumbing、MR1 outcome settlement 和 Daily Review；PostgreSQL 16 是 Runtime、Journal、Repository、Replay、账户、Position 与 Risk 的唯一持久化数据库权威。
...
- WP-DECISION-01 已在现有 Continuous Runtime 下增加唯一 `DECISION_SYSTEM` 子链：14:30–14:55 Daily Summary、Manual Account Observation、Fill-derived Reconciliation、Research Portfolio Proposal 和独立 PostgreSQL Risk Decision。它只提供研究/人工决策支持，不创建 Order、Fill 或 Position 变化。
...
- [Daily Decision System Runbook](docs/operations/Daily-Decision-System-Runbook.md)
- [Historical PostgreSQL Free-Data Migration Matrix](docs/architecture/audits/PostgreSQL-Free-Data-Migration-Matrix.md)
...
数据库运行从 Git 忽略的 `.env` 或显式 CLI 参数读取 `MARKET_REGIME_ALPHA_DATABASE_URL`。本地 bootstrap、备份、恢复、Replay 和 forward-repair 流程见 [PostgreSQL Authority Runbook](docs/operations/PostgreSQL-Authority-Runbook.md)。缺少或无法连接 PostgreSQL 会 fail closed，不存在其他数据库回退路径。
