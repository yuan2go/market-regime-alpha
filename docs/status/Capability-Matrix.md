...
Verification.
WP-15 subsequently executed one real recorded BaoStock Provider gate at merged
main `8067a4be74f697a01aaa996465c10ed5b45b5a7f`. The complete ten-floor
Decision is `REJECTED`; no qualified visibility or downstream Formal Campaign
Authority was created. WP-16 then found no actually accessible inspected
Product with direct recorded `F/F` evidence for historical availability and
revision finality. It stopped before engineering or Protocol registration;
access-blocked `B/?` products are not classified as incapable. Remote CI remains
`BLOCKED_BY_REPOSITORY_CONFIGURATION / NOT_RUN` because repository Actions are
disabled. Nothing here admits a Provider or supplies Formal PIT, Alpha, broker,
...
| Provider qualification | Market owns immutable purpose-specific Protocol/requirements/finality/Decision/Capture/Result rosters and source-specific qualified visibility; the first real BaoStock `HISTORICAL_PIT` scope has a complete `REJECTED` Decision; WP-16's corrected `F/X/?/B` feasibility matrix found no accessible Product with both P0 floors recorded as `F`, so no new Protocol or Decision exists | Market & PIT | `MECHANICS_READY / WP14_EXIT_GATE_PASS / WP15_SCOPE_REJECTED / WP16_GATE_A_BLOCKED / PROVIDER_QUALIFIED_NO` |
| Formal PIT/Dataset | exact Campaign-bound admitted recorded-provider Decision plus typed cutoff-visible sources are mandatory; the rejected WP-15 scope created zero visibility/Dataset/Campaign rows, and WP-16 stopped before implementation because external P0 evidence is absent; no current/latest, caller assertion, or reconstruction path exists | Market & PIT + Research & Qualification | `MECHANICS_READY / BLOCKED_BY_EXTERNAL_PROVIDER_EVIDENCE / FORMAL_PIT_NOT_PROVEN / NOT_CUT_OVER` |
...
| Formal Research Campaign | immutable Target/hypothesis/baseline/Provider/FIT/VALIDATION/LOCKED_OOS/Evaluation/Qualification/cost predeclaration; actual complete Partition/Experiment binding; exact Decision and Due Runtime profiles; protected PostgreSQL-time zero-access opening; WP-15 correctly stopped after its Provider gate was rejected and WP-16 found no accessible P0 `F/F` replacement evidence | Research & Qualification using existing Runtime/owners | `FORMAL_RESEARCH_ENGINEERING_READY / WP14_EXIT_GATE_PASS / WP15_STOPPED_AT_PROVIDER_GATE / WP16_GATE_A_BLOCKED / NO_EMPIRICAL_PROMOTION` |
