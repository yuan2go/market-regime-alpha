...
| Alpha Correctness / Independent Reproduction | `IMPLEMENTED_NOT_PROVEN` | PR #74 adds an independent BaoStock Raw parser/normalizer plus Normalized→Feature/Target replay | PR #74 targeted tests were not run; original physical bytes remain unavailable; top-level `INCONCLUSIVE`; Alpha not proven | Target-test the kernel, then separately reacquire and execute the frozen suite | P0 / engineering then external data |
| External Validation | `IMPLEMENTED_NOT_PROVEN` | Canonical `ResearchExperimentDefinition` freezes exactly one dimension; migration 093 persists the exact `TEMPORAL_VALIDATION_V1` Calendar/window identities | Contamination audit `PASS`; outcomes not read; PR #74 tests and campaign are `NOT_RUN`; `EXTERNALLY_VALIDATED=false`, `FORMAL_OOS=false` | Target-test owner/reload; execute only after `CORRECTNESS_SUPPORTED` | P0/P1 / engineering then evidence |
...
| Conditional Prediction / Strategy Input | `IMPLEMENTED_NOT_WIRED` | Migration 094 and typed domains own/reload pre-Strategy Risk and Opportunity; the conditional forecast kernel exposes availability embargo, uncertainty/minimum-sample/model comparison and exact bindings | No canonical producer records those facts; generic JSON source introspection still substitutes for typed Signal/Forecast/Context/Model reload; conditional consumer remains inactive; raw scores are not probabilities | Derive from existing owners, persist/reload exact typed lineage, then run economics/calibration evidence | P0 / authority + evidence |
| Daily Alpha Prediction | `IMPLEMENTED_NOT_WIRED` | Migration 095 admits an immutable terminal Continuous child and PostgreSQL snapshot authority | Evidence admission can cross-mix experiments; Path Forecast semantics are incomplete; Context is mislabeled as validated factor contribution; PR #74 tests are `NOT_RUN` | Bind one explicit evidence root, typed Forecast projection and exact upstream reload | P0 / engineering |
| T+1 automatic settlement | `IMPLEMENTED_NOT_WIRED` | Existing `FreeDataSettlementOperator` is invoked only by the Continuous post-close path | Selection by `trading_date` does not bind one exact Daily snapshot/run/tick; ambiguity must fail closed | Persist exact snapshot ID/hash through immutable Outcome lineage and keep orchestration in the existing Continuous application boundary | P0 / engineering |
...
| Strategy Registry / Runtime | `IMPLEMENTED_AND_WIRED` | Active V1 `OVERNIGHT`/`SWING_STATE` retain identity in one bounded runtime; dormant conditional contracts are `RESEARCH/SHADOW` and fail closed | Conditional producer composition is incomplete and economic value unproven | Produce typed Risk/Opportunity without activating an unsupported challenger; keep one runtime | P0/P1 |
