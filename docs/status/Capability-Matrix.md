...
| Research Evaluation | legacy experiment/campaign/evaluation owners remain canonical until cutover; the target draft implements three narrow UoWs, session-based purge/embargo, PIT-safe exact revision acquisition, global first-access ordinals, complete observations and metric-member rosters, but WP-11Q found that canonical composition is absent, Partition calendar isolation is not exchange-complete, and Experiment registration persists only one binding rather than a closed ordered roster | Research & Qualification | `WP11_IMPLEMENTED_DRAFT / CORRECTNESS_CLOSURE_REQUIRED / ENGINEERING_QUALIFICATION_PENDING / NOT_CUT_OVER` |
