...
Authority was created. Remote CI remains
...
| Provider qualification | Market owns immutable purpose-specific Protocol/requirements/finality/Decision/Capture/Result rosters and source-specific qualified visibility; the first real BaoStock `HISTORICAL_PIT` scope has a complete `REJECTED` Decision because availability/finality/Outcome-path floors fail and other PIT evidence is insufficient | Market & PIT | `MECHANICS_READY / WP14_EXIT_GATE_PASS / WP15_SCOPE_REJECTED / PROVIDER_QUALIFIED_NO` |
| Formal PIT/Dataset | exact Campaign-bound admitted recorded-provider Decision plus typed cutoff-visible sources are mandatory; the rejected WP-15 scope created zero visibility/Dataset/Campaign rows, and no current/latest, caller assertion, or reconstruction path exists | Market & PIT + Research & Qualification | `MECHANICS_READY / BLOCKED_BY_PROVIDER_GATE / FORMAL_PIT_NOT_PROVEN / NOT_CUT_OVER` |
...
| Formal Research Campaign | immutable Target/hypothesis/baseline/Provider/FIT/VALIDATION/LOCKED_OOS/Evaluation/Qualification/cost predeclaration; actual complete Partition/Experiment binding; exact Decision and Due Runtime profiles; protected PostgreSQL-time zero-access opening; WP-15 correctly stopped before predeclaration after its Provider gate was rejected | Research & Qualification using existing Runtime/owners | `FORMAL_RESEARCH_ENGINEERING_READY / WP14_EXIT_GATE_PASS / WP15_STOPPED_AT_PROVIDER_GATE / NO_EMPIRICAL_PROMOTION` |
