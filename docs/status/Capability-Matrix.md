...
| Alpha Correctness / Independent Reproduction | `IMPLEMENTED_AND_WIRED` | Historical service reloads exact Feature/Outcome/normalized owners, opens physical bytes and rebuilds Feature/Target/placebo/redundancy/block-inference proof before Evidence persistence; execution proxies are independently selected from exact bar order, and any mismatch fails closed | Focused synthetic/unit proof only; physical package unavailable, so `PHYSICAL_REPRODUCTION_NOT_ESTABLISHED`; Alpha not proven | Reopen physical package and run the frozen proof before admitting a real hypothesis | P0 / external data |
| External Validation | `IMPLEMENTED_NOT_PROVEN` | Canonical `ResearchExperimentDefinition` freezes exact hypothesis/thresholds and exactly one Temporal, Universe or Provider change; evaluator binds Feature config/DecisionTime, reloads the Panel-linked Outcome and hash-valid Strategy Economics result, derives the PIT ceiling, uses fractional tie boundaries and emits full diagnostics | No new external dataset executed; `EMPIRICALLY_EXECUTED=false`, `EXTERNALLY_VALIDATED=false`, `FORMAL_OOS=false` | Execute only after real `CORRECTNESS_SUPPORTED` evidence | P1 / evidence |
...
| Conditional Prediction / Strategy Input | `IMPLEMENTED_NOT_WIRED` | Existing PathForecast median is the empirical baseline; frozen regularized challenger exposes per-fold availability embargo, uncertainty/minimum-sample/model comparison and exact bindings; Runtime requires owner-authority reload for any conditional opportunity | Focused engineering tests only; no concrete PostgreSQL pre-Strategy Risk resolver exists, so the conditional consumer intentionally fails closed; the circular post-Portfolio RiskDecision builder was retired; raw barrier scores are not probabilities | Define one pre-Strategy Risk owner and owner-resolving consumer, then run economics/calibration evidence | P2 / authority + evidence |
...
| Strategy Registry / Runtime | `IMPLEMENTED_AND_WIRED` | Active V1 `OVERNIGHT`/`SWING_STATE` retain identity in one bounded runtime; the dormant conditional contract is parsed and fail-closed, but is not registered as an active canonical owner-resolved path | Conditional owner composition is incomplete and economic value unproven | Define the pre-Strategy Risk owner/resolver before conditional activation; keep one runtime | P0/P1 |
