...
| Alpha Correctness / Independent Reproduction | `IMPLEMENTED_NOT_PROVEN` | Normalized→Feature/Target independent replay exists, but Raw→independent-normalization does not yet exist | Original physical bytes unavailable; top-level `INCONCLUSIVE`; Alpha not proven | Reacquire under a distinct owner, compare Raw→Normalized and independently normalized→Feature/Target, then run the frozen suite | P0 / external data |
| External Validation | `IMPLEMENTED_NOT_PROVEN` | Canonical `ResearchExperimentDefinition` freezes exact hypothesis/thresholds and exactly one dimension; `TEMPORAL_VALIDATION_V1` now freezes start `2025-07-15`, 126 owner-derived sessions and the unchanged hypothesis | Contamination audit `PASS`; outcomes not read; `EMPIRICALLY_EXECUTED=false`, `EXTERNALLY_VALIDATED=false`, `FORMAL_OOS=false` | Persist exact Calendar session identities/hash, then execute only after `CORRECTNESS_SUPPORTED` | P0/P1 / evidence |
