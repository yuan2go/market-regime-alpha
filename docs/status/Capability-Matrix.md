...
> **Code Evidence:** Canonical runtime/migration implementation and focused tests on the development branch, with final branch gate pending; H4.5 hardened implementation checkpoint `b1d6533a0b3b1bbd9e180c7f6864b3be8dbd2254`; H6 hardened implementation checkpoint `654e025b97c5d9553d7614b4b5be0898272aacbc`
...
| Role-specific model migration contracts | IMPLEMENTED_DEVELOPMENT_BRANCH | `features/model_contracts.py`, `research/model_contracts.py`, `signals/model_contracts.py`, `decision/model_contracts.py` | Focused structural/semantic contract tests; whole-branch gate pending | No universal Model base; Signal/Decision cannot express order execution | Migrated model implementations and validation absent | Apply through WP-MIG-01 one observable at a time |
| Decimal simple moving-average migration example | IMPLEMENTED_RESEARCH_ONLY_DEVELOPMENT_BRANCH | `features/technical/moving_average.py`, `features/artifact.py`, Legacy MA adapter | Focused purity, missingness, Reader and replay tests; whole-branch gate pending | One unvalidated Feature example, not a Signal or trade action | Broader Moving Average family and formal validation absent | Continue under WP-MIG-01 without changing Legacy thresholds |
| Legacy/New differential harness | IMPLEMENTED_DEVELOPMENT_BRANCH | `migration/comparison/**`, `migration/legacy/normalization/**` | Focused seven-class classification and comparison replay tests; whole-branch gate pending | Comparison evidence only; unknown differences remain `NOT_COMPARABLE` | Model-specific comparison policies and qualified datasets absent | Add policy/invariants for each WP-MIG-01 observable |
| Canonical-to-Legacy import boundary | IMPLEMENTED_DEVELOPMENT_BRANCH | `tests/architecture/test_legacy_import_boundary.py`, `migration/legacy/**` | AST import-boundary test exists; whole-branch gate pending | Legacy may be called only through isolated adapters/compatibility roots | Legacy God Objects still exist | Preserve the guard and retire only after replacement evidence |
...
| Lifecycle Runtime Journal | IMPLEMENTED_DEVELOPMENT_BRANCH_FINAL_GATE_PENDING | `application/canonical_lifecycle/{contracts,repositories,sqlite_repository}.py`, migration 011 | Focused migration/idempotency/history/recovery tests exist; whole-branch gate pending | Single-machine cross-domain reference journal; domain objects remain in existing authorities | No lease owner, distributed scheduler or PostgreSQL parity | Complete branch gate, then add H8 operational control plane |
| Canonical Decision Lifecycle Runner | IMPLEMENTED_FAIL_CLOSED_DEVELOPMENT_BRANCH | `application/canonical_lifecycle/runner.py`, `stages/**` | Focused state, recovery and stage-adapter tests exist; whole-branch gate pending | Research path blocks at unvalidated Entry; H4 continuation observes manual boundaries only | Entry validation, durable H7 authority and sustained operation absent | Keep blockers explicit; do not manufacture stage inputs |
| Lifecycle read-only replay | IMPLEMENTED_DEVELOPMENT_BRANCH | `application/canonical_lifecycle/replay.py` | Focused deterministic/read-only checks exist; whole-branch gate pending | Verifies journal/config/Reader hashes; repository-only objects may be `NOT_COMPARABLE` without injected authority | No external trusted producer/signature | Extend safe Reader registry as authorities mature |
| Canonical lifecycle CLI | IMPLEMENTED_DEVELOPMENT_BRANCH | `cli/run_canonical_lifecycle.py`, `cli/replay_canonical_lifecycle.py` | Focused start/idempotency/resume/replay/safety JSON tests exist; whole-branch gate pending | Structured local CLI; starts research lifecycle and never creates ManualTrade/Fill/order or invokes Broker | No risk-continuation constructor CLI and no operator authentication/control plane | Preserve stable exits and explicit authority binding; extend only with complete typed inputs |
...
For the canonical-runtime development branch, final `pytest`, Ruff, mypy and
package-build results are **PENDING** and are not represented by the historical
