...
| Alpha Correctness / Independent Reproduction | `EXECUTED / CORRECTNESS_FAILED` | Independent package normalization, Normalized→Feature/Target replay, frozen controls and 2,000-iteration inference executed through `historical-phase-ii` | 37,367 Targets supported, 425 partial, 8 failed with `PERSISTED_TARGET_SOURCE_NOT_REPRODUCIBLE`; Alpha not proven | Preserve failures and diagnose through a new protocol revision | P0 / negative evidence |
| External Validation | `BLOCKED_BY_CORRECTNESS / NOT_RUN` | Canonical frozen Temporal Experiment remains unchanged and typed-gated | Correctness did not support admission; no official External Outcome was consumed; `EXTERNALLY_VALIDATED=false` | Do not execute for this failed Experiment | Closed for this Experiment |
