...
| Alpha Correctness / Independent Reproduction | `MISSING` | Current replay reloads canonical PostgreSQL owners but does not independently recompute persisted Feature/Target values from source bars | Physical package is unavailable; owner replay is not physical reproduction | WP-ALPHA-CORRECTNESS-01 before any external-validation claim | P0 |
| External Validation | `MISSING` | Existing Experiment machinery can freeze owners, but no single-dimension external-validation contract/evaluator is active | No new external dataset executed; `EXTERNALLY_VALIDATED=false` | WP-ALPHA-RESEARCH-02 capability, then separately executed evidence | P1 |
| Context Conditional Research | `IMPLEMENTED_NOT_PROVEN` | WP-01 records Context slices but does not enforce session-level versus cross-sectional interpretation as a contract | Market/Theme are session selectors; Capital is unavailable/public proxy | WP-ALPHA-CONTEXT-01 typed conditional evaluation | P1 |
...
| Layered Candidate Policy | `MISSING` | Incumbent Candidate combines required factor availability with the hard-integrity projection; no versioned Incumbent/Challenger comparison contract exists | Missing `return_3`/`amount_ratio_5` can incorrectly appear as universal integrity failure | WP-CANDIDATE-POLICY-02 | P1 |
...
| Conditional Prediction / Strategy Input | `IMPLEMENTED_NOT_WIRED` | Empirical Path Forecast and regularized-linear primitives exist, but StrategyRuntimeInput currently binds Candidate directly and omits Signal/Forecast/Context/Risk/model lineage | Candidate-only Strategy bypass remains possible | WP-PREDICTION-01 explicit requirement contract and runtime wiring | P2 |
...
| Statistical Validity / Multiple Testing | `ENGINEERING_ONLY` | Existing framework applies BH-FDR to one complete 62-hypothesis WP-01 family | 28/49 numeric Factors significant (3 positive, 25 negative); no qualified Locked-OOS result | Re-freeze family for disjoint WP-02 validation | P1/P2 |
