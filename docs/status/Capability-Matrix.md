...
| Canonical Market Data authority | IMPLEMENTED_RESEARCH_ONLY_ON_FEATURE_BRANCH | `market_data/**` | Contract, PIT/adjustment, partition, tamper, atomic publish and replay tests | Exploratory source authority; formal PIT not established | No qualified operational archive | Connect qualified Provider archives without changing authority ceilings |
| Feature materialization and Bundle | IMPLEMENTED_RESEARCH_ONLY_ON_FEATURE_BRANCH | `features/{spine,materialization_v2,v2_contracts}.py`, `features/technical/**` | Six-family, coverage, serial/parallel identity, tamper, crash and replay tests | Observable evidence only | Parameters and real-provider coverage unvalidated | Feed H9 feature ablation and qualified operations |
...
| Technical observable families | IMPLEMENTED_RESEARCH_ONLY_ON_FEATURE_BRANCH | `features/technical/catalog.py`, `observables.py` | Decimal reference/Warmup/missingness/MACD invariant tests | Price, MA/EMA, MACD, Volume/Amount, VWAP and overheat are not trade actions | Formal incremental value absent | Validate under H9; do not promote by engineering completion |
| Legacy/New differential harness | IMPLEMENTED_ON_FEATURE_BRANCH | `migration/comparison/**`, `migration/legacy/adapters/technical_observables.py` | Seven classifications, per-family policies, invariant failure, package/replay tests | Comparison evidence only; amount may be `NOT_COMPARABLE` | Qualified comparison datasets absent | Add Force/Chan/Tuishen policies under WP-MIG-01B |
...
| Signal Feature input V2 | IMPLEMENTED_RESEARCH_ONLY_ON_FEATURE_BRANCH | `signals/input_assembly.py`, `signals/v2.py`, canonical Signal stage | Five-factor scope/freshness/missingness/lineage, Journal receipt and recomputation replay tests | Non-empty factors are not predictive evidence or Entry | Parameters not validated | Add incremental-value and OOS validation |
