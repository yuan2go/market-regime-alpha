...
| Lifecycle read-only replay | IMPLEMENTED_LOCAL_ENGINEERING_GATE_VERIFIED | `application/canonical_lifecycle/replay.py` | Focused deterministic/read-only checks plus local repository-wide gate | Verifies journal/config/Reader hashes; repository-only objects may be `NOT_COMPARABLE` without injected authority | No external trusted producer/signature | Extend safe Reader registry as authorities mature |
...
tests, the full pytest suite (1952 tests collected), Ruff, mypy over 295 source
