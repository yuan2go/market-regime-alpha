...
| WP-ALPHA-CORRECTNESS-02 Target closure | `DESIGN_FROZEN / CODE_NOT_STARTED / RERUN_NOT_RUN` | ADR/protocol freeze separate Decision, Outcome-window and derived-metric state plus new identities and row-level Evidence; no owner/schema/runtime change exists yet | Eight predecessor rows are located: Decision `UNAVAILABLE`, T+1 path `COMPLETE`, dependent metrics `UNAVAILABLE`; this is diagnosis, not new correctness Evidence | Implement/test/persist under a new revision, rerun Discovery only, then issue an independent Go/No-Go | P0 / active design |
...
| Factor Catalog / Extraction | `IMPLEMENTED_AND_WIRED` | Panel v2 projects all 70 canonical outputs; 49 numeric hypotheses use the shared exact-rational tie-aware kernel | WP-02's frozen three-factor higher-direction challenger is significantly adverse in Discovery (RankIC -0.091138, BH-FDR 0.001732) and is rejected; no direction flip | Preserve negative evidence; any reversed-direction family requires a new Experiment | Closed for this Experiment |
| Alpha Correctness / Independent Reproduction | `EXECUTED / CORRECTNESS_FAILED` | Independent package normalization, Normalized→Feature/Target replay, frozen controls and 2,000-iteration inference executed through `historical-phase-ii` | 37,367 Targets supported, 425 partial, 8 failed with `PERSISTED_TARGET_SOURCE_NOT_REPRODUCIBLE`; exact root is an invalid suspended-Daily Decision fallback while all 8 T+1 paths are complete | Preserve immutable failure; implement the frozen revision without changing the hypothesis | P0 / negative evidence |
| External Validation | `BLOCKED_BY_CORRECTNESS / NOT_RUN` | Canonical frozen Temporal Experiment remains unchanged and typed-gated | Correctness did not support admission; no official External Outcome was consumed; `EXTERNALLY_VALIDATED=false` | Do not execute for the failed Experiment. A new revision requires correctness support and a separate explicit `GO`; no automatic admission | Closed for this Experiment |
...
| Transparent Factor/Gate Baseline | `IMPLEMENTED_NOT_PROVEN` | WP-01 binds full Panel, common multi-K/quintile/stability/economics diagnostics and matched-session Gate contrasts to Golden V2 ranking | WP-01 is exploratory positive; the later frozen WP-ALPHA-PROOF-02 hypothesis is adverse. All Gates remain unqualified; PIT/OOS/Production floors are false | Discovery-only correctness repair and sign attribution; no External survivor is currently admitted | P0/P1 |
...
| Statistical Validity / Multiple Testing | `ENGINEERING_ONLY` | Existing framework applies BH-FDR and now exposes its moving-block mean interval to correctness/external diagnostics with block-length sensitivity | WP-01 found 28/49 significant Discovery Factors; WP-ALPHA-PROOF-02's frozen challenger is significantly adverse. No qualified Locked-OOS result exists | Preserve family/directions and rerun only the frozen Discovery correctness comparison under new Target semantics | P1/P2 |
