...
| Alpha Correctness / Independent Reproduction | `IMPLEMENTED_AND_WIRED` | Historical correctness checker recomputes three intraday Factors and T+1 10:30 Target from normalized bars; content-addressed aggregate proof requires every Factor/Target plus physical lineage; mismatch fails closed | Focused synthetic/unit proof only; physical package unavailable, so `PHYSICAL_REPRODUCTION_NOT_ESTABLISHED`; Alpha not proven | Reopen physical package and run the frozen proof before admitting a real hypothesis | P0 / external data |
| External Validation | `IMPLEMENTED_NOT_PROVEN` | Canonical `ResearchExperimentDefinition` freezes exact hypothesis/thresholds and exactly one Temporal, Universe or Provider change; evaluator recomputes score/Target/entry economics, uses fractional tie boundaries, binds the input set and emits full diagnostics | No new external dataset executed; `EMPIRICALLY_EXECUTED=false`, `EXTERNALLY_VALIDATED=false`, `FORMAL_OOS=false` | Execute only after real `CORRECTNESS_SUPPORTED` evidence | P1 / evidence |
...
| Conditional Prediction / Strategy Input | `IMPLEMENTED_AND_WIRED` | Existing PathForecast is the empirical baseline; frozen regularized challenger exposes uncertainty/minimum-sample/model comparison and exact train/validation bindings; incumbent V1 identities stay stable while conditional V2 requires content-addressed Candidate/Signal/Forecast/Context/Risk/Model lineage | Focused engineering tests only; raw barrier scores are not probabilities; no Strategy economics or calibration proof | Real Candidate survivor and separate economic/calibration validation | P2 / evidence |
...
| Strategy Registry / Runtime | `IMPLEMENTED_AND_WIRED` | `OVERNIGHT`, `SWING_STATE` and dormant `CONDITIONAL_PREDICTION` share one bounded runtime; V1 incumbents retain identity and Forecast-required lineage fails closed on missing/inactive/wrong-version input | Engineering/runtime semantics exist; economic value unproven | Activate conditional family only after upstream evidence; keep one runtime | P0/P1 |
