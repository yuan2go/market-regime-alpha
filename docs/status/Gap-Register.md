...
| Pre-Strategy Risk/Opportunity production | Shared semantics bind typed account, Position/exposure, liquidity, restriction, available quantity and configured Risk limits; Continuous records/reloads exact Opportunity when explicitly configured, while Historical rejects material without equivalent Account facts | Conditional Strategy economics and admission evidence are `NOT_RUN` |
...
| Prediction→Outcome exact lineage | Migration 096 freezes snapshot/run/tick and Candidate/Signal/Forecast/Strategy diagnostic lineage; application settlement is idempotent/replayable and ambiguous scope fails closed | Prospective proof remains absent |
