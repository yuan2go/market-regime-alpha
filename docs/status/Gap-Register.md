...
| Formal PIT Data Authority | DELIVERED_WP_PIT_01_LOCAL_ENGINEERING_GATE | Migration 028 and the Data bounded context own explicit Source qualification/suspension, bitemporal fact revisions, server-ingest-safe as-of selection, immutable pass/rejection artifacts, revision-pinned replay and the guarded existing-governance bridge | Real qualified Provider archives and complete historical materialization remain external | A real Provider archive is explicitly source-qualified, all required facts reconstruct without leakage, and the immutable evidence enters Model Governance without automatic qualification |
