...
| H8 sustained Shadow operations | P1 | `SHADOW` now persists an account-neutral prospective Summary under the Continuous Lease/fence and cannot create Order/Fill/Broker/Position mutations; operator acknowledgement, metrics, alerts, T+1 Outcome and sustained evidence remain absent | prospective Summary operation plus outcome boundary | scheduled Shadow operation proves consecutive frozen T decisions, T+1 outcomes, recovery drills and replay without changing authority ceilings |
| Continuous Runtime production operation | P1-EVIDENCE | The single Continuous owner now executes staged BaoStock/Tencent `run-due`, mode-specific selection, Summary, material reuse and recovery on PostgreSQL with recorded providers | exact-window live rehearsal, authenticated scheduler and operating environment | consecutive bounded real-data runs recover from drills, preserve last-valid Evidence and never raise Entry/Broker authority |
| Daily Decision operating evidence | P1-EVIDENCE | Strict Production account decisions remain separate; migration 029 adds immutable Research/Shadow Summary with ordered partial-stage evidence, model receipts, fencing and replay | explicit Research/Shadow model assignments, exact-window Provider evidence and sustained scheduler | consecutive prospective summaries and T+1 outcomes replay without Position mutation, authority inflation or stale-fence writes |
...
| Free-data on-window operation | P0-EXTERNAL | recorded-provider PostgreSQL E2E reaches Research/Shadow Summary; the latest real BaoStock/Tencent attempt was after-window and correctly blocked | trading-day scheduling and Provider availability | Exact-SHA 14:30/14:54/14:55 staged operation publishes and replays a Summary without late data or fallback |
| Free-data staged acquisition | DELIVERED_ENGINEERING_LIVE_UNOBSERVED | one `run-due` entry durably freezes BaoStock history/status, then Tencent quote/static features, then executes the 14:55 Continuous Tick; recorded tests prove single acquisition and restart reuse | exact-window Provider rehearsal | real staged run preserves retrieval/availability deadlines and reaches Summary idempotently |
...
- PostgreSQL-only settings/composition, native bounded repositories, migrations
  001–029, credential-free runtime bindings, isolated-schema replay and
