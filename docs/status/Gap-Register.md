...
| H8 sustained Shadow operations | P1 | `SHADOW` now executes the same Stateful chain and persists a prospective Summary with real owner lineage under the Continuous Lease/fence; it cannot create Order/Fill/Broker/Position mutations. T+1 Outcome, operator acknowledgement, metrics, alerts and sustained evidence remain absent | prospective Summary operation plus outcome boundary | scheduled Shadow operation proves consecutive frozen T decisions, T+1 outcomes, recovery drills and replay without changing authority ceilings |
...
| Daily Decision operating evidence | P1-EVIDENCE | Strict Production account decisions remain separate; Summary V2 binds real State Receipt/CandidateSet, typed outcomes/timestamps, verified model receipts, fencing and replay | exact-window Provider evidence and sustained scheduler | consecutive prospective summaries and T+1 outcomes replay without Position mutation, authority inflation or stale-fence writes |
...
| Free-data on-window operation | P0-EXTERNAL | recorded-provider PostgreSQL E2E reaches non-empty Stateful `RESEARCH_CANDIDATE` in RESEARCH/SHADOW; the latest real BaoStock/Tencent attempt was after-window and correctly blocked | operational ETF/Theme/Capital supplemental producer, trading-day scheduling and Provider availability | Exact-SHA 14:30/14:54/14:54:59/14:55 staged operation publishes and replays a non-empty Summary without late data or fallback |
| Free-data staged acquisition | DELIVERED_ENGINEERING_LIVE_UNOBSERVED | one `run-due` entry freezes BaoStock history/status, Tencent quote/static features, reserves the 14:55 Tick one second early, runs bounded Candidate minute acquisition and does not finalize Summary before DecisionTime; recorded tests prove restart reuse | exact-window Provider rehearsal | real staged run preserves request/response/deadline and stage completion times and reaches Summary idempotently |
...
| Theme and Capital observations | P1 | V0 plus deterministic lifecycle State mechanics run from typed fixtures/supplemental artifacts; no qualified operational producer exists | Qualified market/ETF/symbol history | Daily materializer produces complete decision-time observations with missingness, mapping and coverage reports |
| Free-data supplemental producer | P1 | An explicit immutable supplemental contract now drives the real ETF/Theme/Capital State and non-empty Dynamic Pool/Candidate path in engineering E2E, but there is no live operational producer; absent config remains typed missing evidence with no fallback | effective-dated taxonomy, ETF mapping and observable-proxy sources | A live immutable supplemental artifact passes coverage/availability/lineage validation without claiming institutional intent |
...
- PostgreSQL-only settings/composition, native bounded repositories, migrations
  001–032, credential-free runtime bindings, isolated-schema replay and
