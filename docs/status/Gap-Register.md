...
| Gap | Priority | Current state | Dependency | Exit condition |
|---|---|---|---|---|
| Real Entry Model | P1-VALIDATION | Independent Candidate-only/+Signal/+Forecast/+Intraday research and qualification machinery is implemented; Canonical Entry still cannot emit `ENTER` | qualified Candidate/Signal/path evidence, Calibration, Formal OOS and Governance approval | qualification evidence satisfies the locked floors before a separately versioned Canonical Entry change |
...
| PathForecast calibration | P1-VALIDATION | Historical Sample Registry and Platt/isotonic/binning mechanics with disjoint Fit/Validation/OOS exist; all current samples/fits remain unqualified | real Formal PIT samples and locked OOS | qualified OOS plus Governance evidence transitions `calibrated=true` |
...
| Holding/Exit parameter validation | P1-VALIDATION | independent Shadow Entry/Fill/Position/Holding/Exit/Outcome and locked validation floors exist without touching real execution authorities | qualified closed Strategy Shadow outcomes, Formal OOS and Governance approval | explicit evidence transitions `holding_exit_validated=true` |
...
- PostgreSQL-only settings/composition, native bounded repositories, migrations
  001–045, credential-free runtime bindings, isolated-schema replay and
