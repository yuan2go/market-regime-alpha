...
| Daily Alpha single-lineage admission | One explicit Candidate admission root reloads and verifies Discovery, Correctness, External Experiment/Hypothesis/Dataset and Candidate owners; any missing/drifted/superseded/non-positive edge returns inactive | No real positive admission exists; Challenger stays inactive |
| Pre-Strategy Risk/Opportunity production | Shared producer binds typed account, Position/exposure, liquidity, restriction, available quantity and configured Risk limits; records/reloads exact Opportunity in Continuous/Historical paths | Conditional Strategy economics and admission evidence are `NOT_RUN` |
...
| Daily Forecast projection | Path output projects owner status/MFE/MAE/quantiles/sample/calibration; Conditional output requires exact owner reload or reports unavailable/data-insufficient; Context and validated Alpha contribution are distinct | No qualified Conditional Forecast exists; no calibrated probability claim |
