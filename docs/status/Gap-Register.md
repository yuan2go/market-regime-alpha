...
| H8 sustained Shadow operations | P1-EXTERNAL | `SHADOW` executes the Stateful chain and now has CAS-controlled Session/Status, immutable frozen Decision, append-only T+1 Outcome and no-trade mutation guards. Recorded data proves only engineering mechanics; no decision/outcome pair is prospective | exact-window live operation, operator acknowledgement, incidents/alerts and consecutive sessions | scheduled Shadow operation proves consecutive frozen T decisions, later real T+1 outcomes, recovery drills and replay without changing authority ceilings |
...
| Metrics, tracing and alerts | P1-OPERATIONS | read-only Runtime trace/metrics now derive Provider/stage latency, retries, deadline, coverage, recovery, fence and Summary outcomes from existing owners; no external exporter, threshold history or alert routing exists | exact-window and sustained Shadow operation | observed operating distributions define versioned thresholds and actionable alerts without entering decisions |
...
| Prospective Outcome sample | P0-EXTERNAL | Summary-scoped T+1 Outcome and frozen Evaluation Dataset authorities are implemented and replayable with recorded data, but prospective sample size remains zero | exact-window frozen Shadow Decisions followed by later real checkpoint acquisition | nonzero consecutive samples satisfy `decision_frozen_at < outcome_available_at` without fixture/replay substitution |
