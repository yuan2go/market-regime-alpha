...
| WP-ALPHA-PROOF-02 Target correctness | Closed diagnosis: all eight failures used a previous-session suspended Daily close because the exact same-session 14:55 five-minute Decision reference was absent/unpriced; all eight T+1 paths are complete | Preserve the failed proof; implement the frozen three-state semantic revision and reproduce it under new identities | P0 / research correctness |
| WP-ALPHA-CORRECTNESS-02 row-level failure Evidence | The eight rows are owner-located, but the immutable predecessor Evidence retains only aggregate detail and the design-time table is intentionally abbreviated | Persist stable canonical detail/index identities, full source IDs/hashes, revisions, SHA and owner lineage; reload/report/replay must verify them | P0 / evidence integrity |
| WP-ALPHA-CORRECTNESS-02 implementation and rerun | ADR/protocol are `FROZEN_DESIGN`; code, migration, regression and new Discovery-only run are not started | Shared semantic policy, v3 Target label compatibility, independent source selection, full validation and new-identity Discovery-only correctness/economics | P0 / active design |
...
| Frozen external execution | Closed fail-closed for the predecessor Experiment: correctness failed, so External was not admitted and no official External Outcome was consumed | Preserve the terminal gate result; any future attempt requires supported new-revision correctness plus an explicit independent `GO` based on unchanged-direction Discovery economics | COMPLETE / BLOCKED_BY_CORRECTNESS |
