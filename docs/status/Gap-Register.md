...
| Free-data on-window operation | P0-EXTERNAL | recorded-provider PostgreSQL E2E reaches non-empty Stateful `RESEARCH_CANDIDATE` in RESEARCH/SHADOW; the latest real BaoStock/Tencent attempt was after-window and correctly blocked | operational ETF/Theme/Capital supplemental producer, trading-day scheduling and Provider availability | Exact-SHA 14:30/14:54/14:55 staged operation publishes and replays a non-empty Summary without late data or fallback |
...
- PostgreSQL-only settings/composition, native bounded repositories, migrations
  001–033, credential-free runtime bindings, isolated-schema replay and
