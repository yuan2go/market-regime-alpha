...
|---|---|---|
| Full exact-SHA engineering regression | Clean `f247ca5d`: 3,997 tests and four subtests PASS, plus 39 focused tests and static/build/docs checks; interrupted `d11405ef` is not PASS | Repeat required gates on the final implementation after hard-cut; locked environment without skip/xfail or relaxed assertions |
...
| Fresh generic two-arm gate | Fresh two-arm Run completes all 25 actions, actual Model and six Evaluations; canonical replay, lineage and full 192-table/Artifact zero-write resume match; report bytes remain stable; v5 receipt-index upgrade passes; all prior failed Runs are retained | Prove independent real comparison and the unchanged-parameter 44-session campaign |
