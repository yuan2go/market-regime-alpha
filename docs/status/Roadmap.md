...
| PostgreSQL and historical compatibility | New-scope backup/restore and exact additive v3 upgrade preserve historical rows; original scope is discontinuous | Exact live DB/OID, prior checksums, verified backup/disk/idle preflight, approved additive route and preserved history |
...
| Fresh generic two-arm gate | Horizon repair is implemented; both failed small Runs are retained, including the concurrent serialization-budget failure | Freeze the same engineering parameters under a fresh identity and complete one FIT/VALIDATION rule/ridge execution sequentially; verify canonical Model, Evaluation and replay |
