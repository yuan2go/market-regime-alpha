...
| PostgreSQL and historical compatibility | New-scope backup/restore and exact additive v3 upgrade preserve historical rows; v4 Feature-parent correction has disposable proof and awaits backed-up restore/operational upgrade; original scope is discontinuous | Exact live DB/OID, prior checksums, verified backup/disk/idle preflight, approved additive route and preserved history |
...
| Fresh generic two-arm gate | Horizon and Evaluation cutoff repairs are exercised; latest small Run completed four Evaluations then failed Model opening on owner-specific Feature hash encodings; all failed Runs are retained | Verify v4 ordered Feature-parent comparison on restored actual FIT inputs; upgrade the backed-up new scope, then freeze unchanged parameters under a fresh identity and complete sequential FIT/VALIDATION Model, Evaluation and replay |
