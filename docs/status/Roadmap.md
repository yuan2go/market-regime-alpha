...
|---|---|---|
| Full exact-SHA engineering regression | Clean `c7df943c`: 3,992 tests and four subtests PASS; current receipt-index correction at `f247ca5d` has 39 focused tests passing and full regression in progress; interrupted `d11405ef` is not PASS | Locked environment, all tests/static/build/docs/import gates without skip/xfail or relaxed assertions |
| PostgreSQL and historical compatibility | New-scope backup/restore and exact additive v3/v4 upgrades preserve all 189 business tables; five archive reconciliations and WP-17P replay match; original scope is discontinuous | Exact live DB/OID, prior checksums, verified backup/disk/idle preflight, approved additive route and preserved history |
| Generic real campaign/report/replay | New canonical archive has 116 actual sessions and 534 captures; first 44-session/four-arm campaign failed Evaluation acquisition after 8,168 Outcomes because Archive cutoff preceded actual settlement | Preserve failed identities and times; verify the Runtime-derived Evaluation cutoff in the small chain, then freeze the same 44-session/four-arm research parameters under a new identity and complete canonical Model/Evaluation/report/replay |
| Fresh generic two-arm gate | Fresh two-arm Run completes all 25 actions, actual Model and six Evaluations; canonical replay, lineage and full 192-table/Artifact zero-write resume match; report bytes remain stable; all prior failed Runs are retained | Preserve failed independent Run `e948d119`; finish backed-up v5 receipt-index upgrade, then prove real comparison and the unchanged-parameter 44-session campaign |
...
| Original WP-17P prospective continuity cannot be reconstructed from the recovered historical snapshot; the new R2 generation has only future windows | WP-18Q-R2 prospective continuity | preserve old-scope discontinuity; execute actual due windows through the existing Runtime, record overdue slices/planning gaps and never backdate or infer sustained value |
