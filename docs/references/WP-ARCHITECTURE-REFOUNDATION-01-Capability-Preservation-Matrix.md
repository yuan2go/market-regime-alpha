...
| **Opportunity** | `trading_opportunities/opportunity_events` and Strategy opportunity/source-binding tables | Decision Support: Opportunity | `CreateOpportunity` reloads Candidate/Signal/Forecast/Context/Strategy/pre-risk evidence | `opportunity` | **PRESERVE/CONSOLIDATE** | One exact Decision-time bundle; Forecast-required contract fails closed; Opportunity cannot create Fill/Position |
