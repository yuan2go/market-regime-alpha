...
# 2. Repository-Aware Migration Principle

The repository is not assumed to be greenfield. Migration must preserve evidence-backed behavior while moving authority to explicit bounded contexts and contracts.

The constitutional sequence is:

```text
Freeze terminology and ownership
→ characterize existing behavior
→ create compatibility boundaries
→ move new work to canonical contracts
→ extract reusable components
→ retire Legacy paths only after replacement evidence and caller migration
```

Current implementation facts, module inventories and phase progress are intentionally excluded from this Constitution. They are maintained in:
