...
| **Candidate** | `candidates/*`, candidate discovery, state candidate artifact, daily summary candidate, Strategy proposals | Selection: Candidate Policy/Component/Set/Candidate/Score Component | `RegisterCandidatePolicy`; `BuildCandidateSet` from an immutable Decision-input Dataset; Candidate funnel/dossier queries | `candidate_policy`, `candidate_policy_component`, `candidate_set`, `candidate`, `candidate_score_component` | **PRESERVE/CONSOLIDATE** | Dataset is sole population and every row has a terminal disposition; strict complete case; identity-neutral arithmetic midrank, exact-rational fixed-weight composite, competition rank and all boundary ties; complete typed matrix and Dataset-owned lineage; empty set valid; no Model/Target/Evidence/Entry dependency |
