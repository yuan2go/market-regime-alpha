...
| Conditional Forecast outputs calibrated probability or qualified Strategy action | False: barrier heads emit raw logits/frequencies, calibration is absent, no non-circular pre-Strategy Risk resolver is wired and Strategy economics were not run | `IMPLEMENTED_NOT_WIRED / CALIBRATED=false / STRATEGY_QUALIFIED=false` |
