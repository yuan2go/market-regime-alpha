...
- **P0 WP-ALPHA-CORRECTNESS-02 — EXECUTED / INCONCLUSIVE / NO-GO** — the new protocol revision separates exact 14:55 Decision reference, observed T+1 path and derived-metric states, persists row-level failure Evidence, and executed the unchanged Discovery-only question under a new Experiment identity. Equivalent-source normalization and all typed Target states reproduce; 78 Feature rows remain partial, target-shift controls dominate, the Factors are partially redundant and the frozen higher-is-better direction remains adverse. External is not admitted.
...
- **P2 WP-PREDICTION-01 — ENGINEERING CLOSED / EVIDENCE INACTIVE** — Candidate→Signal→conditional Forecast→Strategy binds typed owner-reloaded Signal, Forecast, Context, Model and owner-derived pre-Strategy Risk/Opportunity semantics in Continuous/Historical adapters; missing DecisionTime Account facts, caller projections and circular post-Portfolio RiskDecision input fail closed, while activation remains blocked by absent correctness/external/calibration/economic evidence.
