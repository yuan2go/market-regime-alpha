...
| `candidates` and candidate discovery | limited arithmetic-midrank/competition-rank invariant clues plus incompatible Model/Target/gate/float/identity-tie paths | permanent `selection` Candidate aggregate consuming real Research Feature Definitions through a Selection-owned port | **APPROVED REDESIGN / IMPLEMENT NEXT**, one five-table Candidate Authority and one write path; no Legacy import, compatibility path, dual write, or fallback |
