...
| Prospective generations / due / terminals / revisions | Market Application using existing Runtime | `prospective_archive_generation`, `prospective_archive_generation_member`, `prospective_archive_slice_schedule`, `prospective_archive_slice_terminal`, `prospective_archive_revision_observation`, `prospective_archive_planning_gap`; PostgreSQL clock and real Runtime fences, never a second scheduler. Slice execution rejects a missing Runtime claim before prospective Provider/normalization/resource-stop effects; downstream owners validate the live fence. Read-only NOT_DUE or already-terminal observations need no new claim. |
