...
| [Candidate Discovery](domains/07-Candidate-Discovery.md) | Own complete eligible cross-section predictions, rankings and explainable recommendation projections. | CandidatePopulation, CandidatePrediction, PredictionRun, CandidateRecommendation | src/market_regime_alpha/candidates/**, WP3 provider-backed Candidate research | daily multi-model Prediction Ledger, structured CandidateRecommendation persistence/API, B2+ challengers |
| [Entry](domains/08-Entry.md) | Own path-aware opening suitability and the incremental comparison against Candidate-only baselines. | EntryAssessment, EntryModelDefinition, EntryPathTarget | src/market_regime_alpha/strategies/entry/** | Entry model/gate, EntryAssessment service, incremental Entry scorecards |
