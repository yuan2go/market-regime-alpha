...
| [Candidate Discovery](domains/07-Candidate-Discovery.md) | Own complete eligible cross-section predictions, rankings and explainable recommendation projections. | CandidatePopulation, CandidatePrediction, PredictionRun, CandidateRecommendation | src/market_regime_alpha/candidates/**, `platform/prediction_run.py`, `daily_decision/recommendation.py`; non-canonical V1 compatibility recommendation under `daily_research/**` | durable/formal Prediction Ledger authority, structured CandidateRecommendation API, regularized B2+ challengers |
| [Entry](domains/08-Entry.md) | Own path-aware opening suitability and the incremental comparison against Candidate-only baselines. | EntryAssessment, EntryModelDefinition, EntryPathTarget | src/market_regime_alpha/strategies/entry/**, exploratory non-ENTER plumbing under `daily_decision/entry.py`; non-canonical V1 compatibility assessment under `daily_research/**` | validated Entry model/gate, canonical EntryAssessment service, incremental Entry scorecards |
...
| [Review and Attribution](domains/13-Review-and-Attribution.md) | Own observed outcomes, layer-specific metrics, failure attribution, rolling scorecards and controlled research proposals. | RecommendationOutcome, DailyReviewReport, FailureAttribution, RollingScorecard | Candidate diagnostics, research artifacts and exploratory MR1 Outcome/DailyReview under `daily_decision/**` | broader-horizon outcome matching, failure taxonomy runtime, rolling scorecards and attribution |
