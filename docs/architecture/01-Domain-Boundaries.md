...
| Trade Decision and Risk | Own TradingOpportunity, TradingThesis, invalidation, portfolio proposal and independent hard-risk permission before execution. | TradingOpportunity, TradingThesis, InvalidationCondition, RiskDecision | thin simulation boundary in `decision/contracts.py`; target design in Architecture 10 | stateful aggregates, repositories, expiry/invalidation, audit, permissions and fail-closed Risk Authority |
