...
## Task 11A.11 correctness hardening

Research lifecycle 的买回到期现在使用显式交易日历/session index；周末或节假日不会消耗
`expiry_trade_days`。reverse-T 的买回恢复 `base_shares`，partial buyback 同步减少
`remaining_allocated_proceeds` 并累计 cycle 成本/PnL；HARD 风险会记录
`BUYBACK_CANCELLED_BY_HARD_RISK` 与 `REVERSE_T_CONVERTED_TO_RISK_REDUCTION`。新增的
research selector action 只在 shadow 记录 `legacy_sell_action/research_sell_action`，没有改变
生产执行。

Builder 现在验证 corporate-action PIT evidence、停牌/eligibility 一致性、每个
`symbol,timestamp` 的行业/benchmark as-of 映射、源单位归一化后的 VWAP，以及 calendar 定义的
session-close bar。Manifest identity 的单位字段在后续 schema 升级前由 Builder quality/artifact
明确记录；正式 dataset manifest schema 需要与 source adapter 一起升级，不能由当前 PoC 伪造。
